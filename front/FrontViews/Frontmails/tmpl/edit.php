	...
	<div id="acym__editor__content" class="grid-x acym__content acym__editor__area <?php echo !empty($data['menuClass']) ? acym_escape($data['menuClass']) : ''; ?>">
		<div class="cell grid-x grid-margin-x align-right margin-bottom-1">
			...
			<input type="hidden"
			       id="acym__mail__type"
			       name="mail[type]"
			       value="<?php echo empty($data['mail']->type) ? acym_escape($data['mailClass']::TYPE_STANDARD) : acym_escape($data['mail']->type); ?>">
			<input type="hidden" name="list_ids" value="<?php echo empty($data['list_id'][0]) ? 0 : acym_escape($data['list_id'][0]); ?>">
            <?php
		...
		<div class="cell grid-x grid-padding-x acym__editor__content__options">
			...
			<div class="cell grid-x acym__toggle__arrow">
				<p class="cell medium-shrink acym__toggle__arrow__trigger">
                    <?php echo acym_escapeHtml(acym_translation('ACYM_ADVANCED_OPTIONS')); ?> <i class="acymicon-keyboard-arrow-down"></i>
				</p>
				<div class="cell acym__toggle__arrow__contain">
					<div class="grid-x grid-padding-x">
                        <?php if (empty($data['multilingual']) && empty($data['abtest'])) { ?>
							<div class="cell grid-x">
								...
								<input id="acym__mail__edit__preheader"
								       ...
								       type="text"
								       maxlength="255"
								       value="<?php echo acym_escape($data['mail']->preheader); ?>">
							</div>
                        <?php } ?>
						...
						<div class="cell medium-auto">
							<label for="acym__mail__edit__custom__header"><?php echo acym_escapeHtml(acym_translation('ACYM_CUSTOM_HEADERS')); ?></label>
							<textarea id="acym__mail__edit__custom__header" name="editor_headers" cols="30" rows="15" type="text">
                            <?php echo acym_escapeHtml($data['mail']->headers); ?>
                        </textarea>
						</div>
					</div>
				</div>
