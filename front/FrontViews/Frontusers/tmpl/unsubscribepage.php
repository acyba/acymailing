...
<div class="acym__unsubscribe__page grid-x grid-padding-x">
		...
		<div class="acym__unsubscribe__image__container cell hide-for-small-only">
            <?php if (!empty($this->config->get('unsubscribe_image'))) { ?>
				<img class="cell padding-3"
				     src="<?php echo acym_escapeUrl($this->config->get('unsubscribe_image')); ?>"
	...
	<div class="acym__unsubscribe__form__container cell <?php echo acym_escape($classContainer); ?>">
		<div class="cell">
			<form action="<?php echo acym_escapeUrl(acym_frontendLink('frontusers')); ?>" name="unsubscribepage" class="acym_front_page acym__unsubscribe__form">
				<fieldset class="grid-x grid-margin-x align-center">
					<legend class="show-for-sr"><?php echo acym_escapeHtml(acym_translation('ACYM_EMAIL_PREFERENCES')); ?></legend>
					<div class="acym__unsubscribe__container cell">
						<div class="acy__unsubscribe__form__header margin-bottom-3">
							<div class="acym__unsubscribe__form__header__image cell padding-bottom-1">
                                <?php if (!empty($this->config->get('unsubscribe_logo'))) { ?>
									<img class="cell" src="<?php echo acym_escape($this->config->get('unsubscribe_logo')); ?>" alt="Unsubscribe Logo">
                                <?php } ?>
							</div>
							<h1 class="acym__title acym__unsubscribe__title margin-bottom-1">
                                <?php if (!empty($this->config->get('unsubscribe_title'))) {
                                    echo acym_escapeHtml($this->config->get('unsubscribe_title'));
                                } else {
                                    echo acym_escapeHtml(acym_translation('ACYM_EMAIL_PREFERENCES'));
                                } ?>
							</h1>
						...
						<div class="acym__unsubscribe__lists__container cell margin-bottom-1">
                            <?php if (empty($data['subscriptions'])) { ?>
								<p><?php echo acym_escapeHtml(acym_translation('ACYM_NO_DATA_TO_DISPLAY')); ?></p>
								<p><?php echo acym_escapeHtml(acym_translation('ACYM_NO_LIST_FOUND_CAN_UNSUB')); ?></p>
                            <?php } else { ?>
											...
											<li class="acym__unsubscribe__list__item grid-x">
												<div class="acym__unsubscribe__lists__listing cell margin-bottom-1">
													<p class="acym__unsubscribe__list__display__name">
                                                        <?php echo acym_escapeHtml(!empty($list->display_name) ? $list->display_name : $list->name); ?>
													</p>
													<p class="acym__unsubscribe__list__description">
                                                        <?php echo !empty($list->description) ? acym_escapeHtml($list->description) : ''; ?>
													</p>
												</div>
							...
							<div class="acym__unsubscribe__reason__section cell margin-bottom-1">
								<div class="acym__unsubscribe__reason__label">
									<p class="acym__unsubscribe__reason"><?php echo acym_escapeHtml(acym_translation('ACYM_SHARE_YOUR_REASONS')); ?></p>
									<span class="acym__optional"><?php echo acym_escapeHtml(acym_translation('ACYM_OPTIONAL')); ?></span>
								</div>
                                <?php if (!empty($data['surveyAnswers'])) {
									...
									<div class="acym__unsubscribe__reason__label acym__custom__reason__label padding-top-1 is-hidden">
										<p class="acym__unsubscribe__reason"><?php echo acym_escapeHtml(acym_translation('ACYM_CAN_YOU_TELL_MORE')); ?></p>
										<span class="acym__optional"><?php echo acym_escapeHtml(acym_translation('ACYM_OPTIONAL')); ?></span>
									</div>
									<input type="text"
							...
							<div class="acym__unsubscribe__actions grid-x align-center">
                                <?php if (count($data['subscriptions']) > 1 || empty($data['subscriptions'])) { ?>
									<button type="button"
                                        ...
                                        <?php echo acym_escapeHtml(acym_translation('ACYM_UNSUBSCRIBE_ALL')); ?>
									</button>
                                <?php }
                                if (empty($data['subscriptions'])) { ?>
									<button type="button"
                                        ...
                                        <?php echo acym_escapeHtml(acym_translation('ACYM_UNSUBSCRIBE')); ?>
									</button>
                                <?php } else { ?>
									<button type="button"
									        class="button button-primary cell margin-bottom-1"
                                        ...
                                        <?php echo acym_escapeHtml(acym_translation('ACYM_UPDATE_PREFERENCES')); ?>
									</button>
                                <?php } ?>
							</div>
						</div>
                    ...
                    if ($this->config->get('display_built_by', 1)) { ?>
						<img class="cell acym__powered__by__image"
						     src="<?php echo acym_escapeUrl(ACYM_IMAGES.'editor/poweredby_black.png'); ?>"
...
<style>
	#acym__unsub__all:hover{
		background-color: <?php echo acym_escapeHtml($data['hoverColor']); ?> !important;
		border-color: <?php echo acym_escapeHtml($data['hoverColor']); ?> !important;
	}
	...
	.switch input:checked ~ .switch-paddle{
		background-color: <?php echo acym_escapeHtml($data['unsubscribeColor']); ?> !important;
		border-color: <?php echo acym_escapeHtml($data['unsubscribeColor']); ?> !important;
	}
	...
	.button-secondary{
		border-color: <?php echo acym_escapeHtml($data['unsubscribeColor']); ?> !important;
		color: <?php echo acym_escapeHtml($data['unsubscribeColor']); ?> !important;
	}
