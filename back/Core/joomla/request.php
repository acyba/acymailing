...
function acym_makeCurlCall(string $url, array $options = []): array
{
    ...
    if ($multipart) {
        foreach ($files as $file) {
            $data[$file['name']] = curl_file_create($file['path'], '', $file['filename'] ?? basename($file['path']));
        }
        if ($contentTypeKey !== '') {
            unset($headers[$contentTypeKey]);
        }
    } elseif (!empty($data)) {
        $isHeaderContentTypeJson = $contentTypeKey !== '' && strtolower(trim($headers[$contentTypeKey])) === 'application/json';
        $dataFormatted = ($method === 'POST' && $isHeaderContentTypeJson) ? json_encode($data) : http_build_query($data);
    }
