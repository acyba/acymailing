<div class="acym__content cell grid-x large-6 margin-bottom-1">
	<div class="cell grid-x margin-y" id="acym__mailbox__edition__information">
		...
		<label class="cell grid-x">
			...
			<input required class="cell medium-7" type="text" name="mailbox[name]" value="<?php echo acym_escape($data['mailboxAction']->name); ?>">
		</label>
		<div class="cell grid-x">
            <?php
            echo acym_switch(
                'mailbox[active]',
                empty($data['mailboxAction']->active) ? 0 : 1,
		...
		<label class="cell grid-x">
			...
			<textarea rows="10" class="cell medium-7" name="mailbox[description]"><?php echo acym_escape($data['mailboxAction']->description); ?></textarea>
		</label>
	</div>
</div>
