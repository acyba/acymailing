...
class Mail extends PHPMailer implements MailerInterface
{
    ...
    public function Send()
    {
        ...
        if (!empty($this->to[0][0]) && include_once rtrim(JPATH_ADMINISTRATOR, $ds).$ds.'components'.$ds.'com_acym'.$ds.'Core'.$ds.'init.php') {
            $mailerHelper = new MailerHelper();

            $recipients = array_keys($this->getAllRecipientAddresses());
            $to = array_shift($recipients);
            $bcc = $recipients;

            $headers = [];
            $customHeaders = $this->getCustomHeaders();
            foreach ($customHeaders as $oneHeader) {
            ...
            foreach ($this->getAttachments() as $attachment) {
                ...
                $attachments[] = (object)[
                    ...
                    'name' => $attachment[2],
                    'url' => str_replace(ACYM_ROOT, ACYM_LIVE, $attachment[0]),
                ];
            }
