...
class ParserState
{
    ...
    public function parseIdentifier(bool $ignoreCase = true): string
    {
        ...
        while (!$this->isEnd() && ($character = $this->parseCharacter(true)) !== null) {
            if (CSSList::preg_match('/[a-zA-Z0-9\\x{00A0}-\\x{FFFF}_-]/Sux', $character) !== 0) {
                $result .= $character;
            } else {
                $result .= '\\' . $character;
            }
    ...
    public function parseCharacter(bool $isForIdentifier): ?string
    {
        if ($this->peek() === '\\') {
            ...
            return CSSList::iconv('utf-32le', $this->charset, $utf32EncodedCharacter);
        }
        if ($isForIdentifier) {
            $peek = \ord($this->peek());
    ...
    public function consumeWhiteSpace(): array
    {
        $comments = [];
        do {
            while (CSSList::preg_match('/\\s/isSu', $this->peek()) === 1) {
                $this->consume(1);
            }
    ...
    public function consumeExpression(string $expression, ?int $maximumLength = null): string
    {
        ...
        if (CSSList::preg_match($expression, $input, $matches, PREG_OFFSET_CAPTURE) !== 1) {
    ...
    private function strsplit(string $string): array
    {
        if ($this->parserSettings->hasMultibyteSupport()) {
            if ($this->streql($this->charset, 'utf-8')) {
                $result = CSSList::preg_split('//u', $string, -1, PREG_SPLIT_NO_EMPTY);
            } else {
