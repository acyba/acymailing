<div class="acym__popup__listing text-center grid-x">
	<h1 class="acym__title acym__title__secondary text-center cell"><?php echo acym_translation('ACYM_LINKS') ?></h1>
	<div class="grid-x medium-12 cell acym__row__no-listing text-left acym_vcenter">
		<div class="grid-x cell medium-5 small-12 acym__listing__title acym__listing__title__dynamics acym__online__link acym_vcenter">
			<label class="small-3 margin-bottom-0" for="acym__popup__online__tagtext"><?php echo acym_translation('ACYM_TEXT') ?>: </label>
	...
	<div class="cell grid-x">
        <?php foreach ($links as $tagname => $tag) {
			...
			<div class="grid-x small-12 cell acym__row__no-listing acym__listing__row__popup text-left<?php echo $disabledClass ?>" <?php echo empty($onclick) ? '' : 'onclick="'.$onclick.'"' ?>
				 id="tr_<?php echo $tagname ?>">
                <?php if (!empty($tag['disabled'])) {
                    echo acym_tooltip(['hoveredText' => $tag['desc'], 'textShownInTooltip' => $tag['tooltip'], 'classContainer' => 'cell']);
                } else { ?>
					<div class="cell small-12 acym__listing__title acym__listing__title__dynamics"><?php echo $tag['desc'] ?></div>
                <?php } ?>
			</div>
        <?php } ?>
	</div>
...
<div class="acym__popup__listing text-center grid-x">
	<h1 class="acym__title acym__title__secondary text-center cell"><?php echo acym_translation('ACYM_INFORMATION') ?></h1>
	<div class="cell grid-x">
        <?php foreach ($information as $infoKey => $info) {
            ...
            ?>
			<div class="grid-x medium-12 cell acym__row__no-listing acym__listing__row__popup text-left<?php echo $disabledClass ?>"
                <?php empty($onclick) ? '' : 'onclick="'.$onclick.'"' ?>
				 id="tr_<?php echo $infoKey ?>">
				<div class="cell medium-12 small-12 acym__listing__title acym__listing__title__dynamics">
                    <?php echo $info['label'] ?> :
                    <?php if (acym_isImageUrl($info['value'])) { ?>
						<img src="<?php echo $info['value'] ?>" alt="Image" style="display: inline-block; max-width: 25px; max-height: 25px; vertical-align: middle;" />
                    <?php } else { ?>
                        <?php echo $info['value'] ?>
                    <?php } ?>
				</div>
