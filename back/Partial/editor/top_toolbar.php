...
<div id="acym__wysid__top-toolbar" class="grid-x cell">
	<div class="cell grid-x" id="acym__wysid__top-toolbar__actions">
		...
		<div class="cell grid-x auto small-4 hide-for-small-only align-center acym_vcenter">
			<p class="cell shrink acym__wysid__top-toolbar__icon margin-right-1"><?php echo acym_escapeHtml(acym_translation('ACYM_PREVIEW')); ?></p>
		...
		<div class="cell grid-x small-4 align-right">
            <?php if (!empty($data['mail']) && $data['mail']->type == $data['mailClass']::TYPE_OVERRIDE) { ?>
				<a href="<?php echo acym_escapeUrl(ACYM_DOCUMENTATION.'main-pages/email-overrides/edition'); ?>"
				   class="cell small-6 medium-shrink margin-bottom-0 acym_vcenter"
				   target="_blank">
					<i class="acymicon-book acym__wysid__top-toolbar__button__icon" data-acym-tooltip="<?php echo acym_escape(acym_translation('ACYM_DOCUMENTATION')); ?>"></i>
				</a>
            <?php } ?>
            <?php
            ...
            if (acym_isAdmin() && ($isCampaign || $isWelcomeOrUnsub) && acym_isAllowed('mails')) {
				...
				<button id="acym__wysid__saveastmpl__button" type="button" class="cell small-6 medium-shrink margin-bottom-0 margin-right-0">
					<i class="acymicon-content-copy acym__wysid__top-toolbar__button__icon"
					   data-acym-tooltip="<?php echo acym_escape(acym_translation('ACYM_SAVE_AS_TMPL')); ?>"></i>
				</button>
            <?php } ?>
            <?php if (acym_isAllowed('mails')) { ?>
				<button id="acym__wysid__test__button" type="button" class="cell small-6 medium-shrink margin-bottom-0">
					<i class="acymicon-paper-plane acym__wysid__top-toolbar__button__icon" data-acym-tooltip="<?php echo acym_escape(acym_translation('ACYM_SEND_TEST')); ?>"></i>
				</button>
            <?php } ?>
			...
			<button id="acym__wysid__cancel__button" type="button" class="cell small-6 medium-shrink margin-bottom-0 margin-right-1">
				<i class="acymicon-ban acym__wysid__top-toolbar__button__icon" data-acym-tooltip="<?php echo acym_escape(acym_translation('ACYM_CANCEL')); ?>"></i>
			</button>
			<button id="acym__wysid__save__button" type="button" class="cell small-6 medium-shrink margin-bottom-0">
				<i class="acymicon-floppy-o acym__wysid__top-toolbar__button__icon" data-acym-tooltip="<?php echo acym_escape(acym_translation('ACYM_APPLY')); ?>"></i>
			</button>
		</div>
	</div>
	<div class="cell grid-x align-left acym_vcenter" id="acym__wysid__top-toolbar__notification">
		<i class="cell shrink" id="acym__wysid__top-toolbar__notification__icon"></i>
