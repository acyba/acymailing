...
<form id="acym_form"
	...
	<div id="acym__followup__condition" class="cell grid-x margin-y align-center acym__content margin-top-2 large-9">
		...
		<input type="hidden" name="trigger" value="<?php echo empty($data['followup']->trigger) ? acym_escape($data['trigger']) : ''; ?>">
		<div class="cell grid-x">
            <?php
            $workflow = $data['workflowHelper'];
            if (empty($data['followup']->id)) $data['workflowHelper']->disabledAfter = 'followupCondition';
            $workflow->display($this->followupSteps, 'followupCondition', true, false, empty($data['followup']->trigger) ? '&trigger='.$data['trigger'] : '');
            ?>
		</div>
		<p class="cell acym__followup__condition__desc padding-left-1"><?php echo acym_escapeHtml(acym_translation('ACYM_FOLLOW_UP_CONDITION_DESC_1')); ?></p>
		<p class="cell padding-left-1"><?php echo acym_escapeHtml(acym_translation('ACYM_FOLLOW_UP_CONDITION_DESC_2')); ?></p>
		...
		<div class="cell grid-x padding-left-1">
            ...
            if (!empty($data['additionalCondition']) || $data['trigger'] == 'user_subscribe') {
                echo '<h5 class="cell acym__title__primary__color padding-left-1">'.acym_escapeHtml(acym_translation('ACYM_SPECIFIC_CONDITIONS_TRIGGER')).'</h5>';
            }
		...
		<div class="cell grid-x">
			...
			<div class="cell auto align-right grid-margin-x grid-x">
				<button type="button" class="cell shrink acy_button_submit button button-secondary" data-task="save" data-step="listing"><?php echo acym_escapeHtml(
                        acym_translation(
                            'ACYM_SAVE_EXIT'
                        )
                    ); ?></button>
				<button type="button" class="cell shrink acy_button_submit button" data-task="save" data-step="followupEmail"><?php echo acym_escapeHtml(
                        acym_translation('ACYM_SAVE_CONTINUE')
                    ); ?>
