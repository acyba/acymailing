...
<?php } else { ?>
	...
	<div class="cell grid-x">
		<div class="grid-x acym__listing__actions cell auto">
            ...
            if (isset($data['campaign_type'])) {
                acym_filterStatus($options, $data['status'], $data['campaign_type'].'_status');
            }
            ?>
		</div>
	...
	<div class="grid-x acym__listing">
		<div class="grid-x cell acym__listing__header">
			...
			<div class="grid-x medium-auto small-11 cell acym__listing__header__title__container">
				<div class="medium-auto small-11 cell acym__listing__header__title">
                    <?php echo acym_escapeHtml(acym_translation('ACYM_CAMPAIGNS')); ?>
				</div>
				<div class="<?php echo $data['campaign_type'] === 'campaigns_auto' ? 'large-1' : 'large-2'; ?> medium-3 hide-for-small-only cell acym__listing__header__title">
                    <?php echo acym_escapeHtml(acym_translation('ACYM_LISTS')); ?>
				</div>
				<div class="large-2 medium-4 hide-for-small-only cell acym__listing__header__title text-center">
                    <?php echo acym_escapeHtml(acym_translation($data['campaign_type'] === 'campaigns_auto' ? 'ACYM_FREQUENCY' : 'ACYM_STATUS')); ?>
				</div>
                <?php if ($data['campaign_type'] === 'campaigns_auto') { ?>
					<div class="large-1 hide-for-small-only hide-for-medium-only text-center cell acym__listing__header__title acym__campaign__auto__generation">
                        <?php echo acym_escapeHtml(acym_translation('ACYM_LAST_GENERATION')); ?>
					</div>
					<div class="large-1 hide-for-small-only hide-for-medium-only text-center cell acym__listing__header__title acym__campaign__auto__generation">
                        <?php echo acym_escapeHtml(acym_translation('ACYM_NEXT_TRIGGER')); ?>
					</div>
                <?php } ?>
                ...
                <?php if (acym_isAdmin()) { ?>
					<div class="large-1 cell hide-for-small-only hide-for-medium-only text-center acym__listing__header__title">
                        <?php
                        echo acym_escapeHtml(acym_translation('ACYM_VISIBLE'));
                        acym_info(
                            [
        ...
        foreach ($data['allCampaigns'] as $campaign) {
            ...
            $specialText = '';
            if (
                is_array($campaign->sending_params)
                && array_key_exists('abtest', $campaign->sending_params)
                && empty($campaign->sending_params['abtest']['abtest_finished'])
                && empty($campaign->draft)
            ) {
			...
			<div class="grid-x cell align-middle acym__listing__row">
				...
				<div class="grid-x medium-auto small-11 cell align-middle acym__campaign__listing acym__listing__title__container">
					<div class="cell medium-auto small-7 acym__listing__title acym__campaign__title">
                        ...
                        ?>
						<a class="cell auto" href="<?php echo acym_escapeUrl($linkCampaign); ?>">
							<h6 class='acym__listing__title__primary acym_text_ellipsis' style="display:flex; align-items:center;">
                                ...
                                if (is_array($campaign->sending_params) && array_key_exists('abtest', $campaign->sending_params)) {
                                    echo '<i class="acymicon-flask acym__selection__card__icon"></i> ';
                                }
                                echo acym_escapeHtml($campaign->name.$specialText);
                                ?>
							</h6>
						</a>
						<p class="acym__listing__title__secondary">
                            ...
                            if (!empty($campaign->sending_date) && (!$campaign->scheduled || $campaign->sent)) {
                                echo acym_escapeHtml(
                                    acym_translation('ACYM_SENDING_DATE').' : '.acym_date($campaign->sending_date, acym_getDateTimeFormat('ACYM_DATE_FORMAT_LC3'))
                                );
                                ...
                                echo acym_escapeHtml(acym_translationSprintf('ACYM_X_CAMPAIGN_GENERATED', $numberCampaignsGenerated));
                            }
                            ?>
						</p>
					</div>
					<div class="<?php echo $data['campaign_type'] === 'campaigns_auto' ? 'large-1' : 'large-2'; ?> acym__campaign__recipients medium-3 small-5 cell">
                        ...
                        if (!empty($campaign->lists)) {
                            ...
                            foreach ($campaign->lists as $list) {
                                ...
                                acym_tooltip(
                                    [
                                        'hoveredText' => '<i class="'.acym_escape($classes).'" style="color:'.acym_escape($list->color).'"></i>',
                            ...
                            if ($counter > 5 && $subscriptionsCount !== 6) {
                                $counter = $counter - 5;
                                echo '<span class="acym__campaign__show-subscription acymicon-stack" data-iscollapsed="0" acym-data-value="'.acym_escape($counter).'">
										<i class="acym__campaign__button__showsubscription acymicon-circle acymicon-stack-2x"></i>
										<span class="acym__listing__text acym__campaign__show-subscription-bt acymicon-stack-1x">+'.acym_escapeHtml($counter).'</span>
									</span>';
                            }
                        } else {
                            echo '<div class="cell medium-12">';
                            echo acym_escapeHtml(acym_translation(empty($campaign->automation) ? 'ACYM_NO_LIST_SELECTED' : 'ACYM_SENT_WITH_AUTOMATION'));
                            echo '</div>';
                        }
                        ?>
					</div>
					<div class="large-2 medium-4 small-11 text-center cell acym__campaign__status">
						<div class="grid-x text-center">
                            ...
                            if ($campaign->sent) {
                                ...
                                echo acym_escapeHtml(acym_translation('ACYM_SENT').' : '.$campaign->subscribers.' '.acym_translation('ACYM_RECIPIENTS'));
                                echo '</span></div>';
                                ...
                                echo acym_escapeHtml(acym_translation('ACYM_SCHEDULED').' : ');
                                acym_tooltip(
                                    [
                                ...
                                echo '<div class="cell acym__campaign__listing__status__controls"><div class="grid-x text-center"><div class="cell auto"></div>';
                                acym_tooltip(
                                    [
                                        'hoveredText' => $target,
                                ...
                                echo '<div class="cell auto"></div></div></div>';
								...
								<div class="cell acym__campaign__listing__status__controls">
									<div class="grid-x text-center">
										<div class="cell auto"></div>

										<div class="acym__campaign__listing__automatic__deactivate grid-x cell xlarge-shrink acym_vcenter"
										     data-campaignid="<?php echo acym_escape($campaign->id); ?>">
											<i class="'.($campaign->active ? 'acymicon-times-circle' : 'acymicon-power-off').' cell shrink show-for-xlarge"></i>
											<span class="cell xlarge-shrink">
											<?php echo acym_escapeHtml(
                                                $campaign->active
                                                    ? acym_translation('ACYM_DEACTIVATE')
                                                    : acym_translation('ACYM_ACTIVATE')
                                            ); ?>
										</span>
										</div>
                                ...
                                echo '<div class="cell acym__campaign__status__status acym__background-color__orange"><span>';
                                echo acym_escapeHtml(acym_translation('ACYM_WAITING_FOR_CONFIRMATION'));
                                echo '</span></div>';
                                // Is a generated campaign that has been disabled
                            } elseif ('generated' == $data['status'] && !$campaign->active) {
                                echo '<div class="cell acym__campaign__status__status acym__background-color__red"><span class="acym__color__white">';
                                echo acym_escapeHtml(acym_translation('ACYM_DISABLED'));
                                echo '</span></div>';
                                // Is a draft
                            } elseif ($campaign->draft) {
                                echo '<div class="cell acym__campaign__status__status acym__campaign__status__draft"><span>';
                                echo acym_escapeHtml(acym_translation('ACYM_DRAFT'));
                                echo '</span></div>';
                            }
                            ?>
						</div>
                    ...
                    <?php if ($data['campaign_type'] === 'campaigns_auto') { ?>
						<div class="large-1 hide-for-small-only hide-for-medium-only cell text-center acym__campaign__auto__generation">
                            ...
                            if (empty($campaign->last_generated)) {
                                ...
                                echo acym_escapeHtml(acym_date($campaign->last_generated, acym_getDateTimeFormat()));
                            }
                            ?>
						</div>
						<div class="large-1 hide-for-small-only hide-for-medium-only cell text-center acym__campaign__auto__generation">
                            ...
                            if (empty($campaign->next_trigger)) {
                                ...
                                echo acym_escapeHtml(acym_date($campaign->next_trigger, acym_getDateTimeFormat()));
                            }
                            ?>
						</div>
                    <?php } ?>
					<div class="large-1 hide-for-small-only hide-for-medium-only cell text-center">
                        ...
                        if (($campaign->sent || $data['campaign_type'] === 'campaigns_auto') && !empty($campaign->subscribers) && isset($campaign->open)) {
                            echo acym_escapeHtml($campaign->open).'%';
                        } else {
                            echo '-';
                        }
                        ?>
					...
					<div class="large-1 hide-for-small-only hide-for-medium-only cell text-center">
                        ...
                        if (($campaign->sent || $data['campaign_type'] === 'campaigns_auto') && !empty($campaign->subscribers) && isset($campaign->click)) {
                            echo acym_escapeHtml($campaign->click).'%';
                        } else {
                            echo '-';
                        }
                        ?>
                    ...
                    <?php if (acym_isTrackingSalesActive()) { ?>
						<div class="large-1 hide-for-small-only hide-for-medium-only cell text-center">
                            ...
                            if (!empty($campaign->tracking_sale) && !empty($campaign->currency)) {
                                ...
                                echo acym_escapeHtml(round($campaign->tracking_sale, 2).' '.$campaign->currency);
                            } else {
                                echo '-';
                            }
                            ?>
                    ...
                    <?php if (acym_isAdmin()) { ?>
						<div class="large-1 hide-for-small-only hide-for-medium-only cell text-center acym__icon__table">
                            ...
                            if ($campaign->visible == 1) {
                                ...
                                $class = 'acymicon-eye-slash acym__color__dark-gray';
                                $newvalue = 1;
					...
					<div class="large-1 hide-for-small-only hide-for-medium-only cell text-center acym__icon__table">
						<a href="<?php echo acym_escapeUrl($linkCampaign); ?>">
							<i class="acymicon-pencil" title="<?php echo acym_escape(acym_translation('ACYM_EDIT')); ?>"></i>
						</a>
						<a>
							<i class="acymicon-content-copy fastActions"
                        ...
                        if (!acym_escape($campaign->draft) && !$isFront) {
                            ?>
							<a href="<?php echo acym_escapeUrl(acym_completeLink('stats&mail_ids='.acym_escape($campaign->mail_id))); ?>">
