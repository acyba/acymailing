...
?>
<form id="acym_form" action="<?php echo acym_escapeUrl(acym_completeLink(acym_getVar('cmd', 'ctrl'))); ?>" method="post" name="acyForm" class="cell grid-x align-center">
	<div id="acym__followup__summary" class="cell grid-x margin-y acym__content margin-top-2 large-9">
		...
		<input type="hidden" name="trigger" value="<?php echo !empty($data['trigger']) ? acym_escape($data['trigger']) : ''; ?>">
		<div class="cell grid-x">
            <?php $data['workflowHelper']->display($this->followupSteps, 'followupSummary'); ?>
		</div>
		...
		<div class="cell grid-x acym__followup__summary__section margin-right-2">
			<h5 class="cell shrink margin-next-1 acym__title acym__title__secondary">
				<b><?php echo acym_escapeHtml(acym_translation('ACYM_TRIGGER')); ?></b>
			</h5>
            <?php
            $blocks = [];
            ...
            foreach ($blocks as $block) {
				...
				<div class="cell grid-x">
					<div class="cell small-5 medium-4 large-2 acym__summary__card text-center">
						<i class="<?php echo acym_escape($block['icon']); ?>"></i>
						<p><?php echo acym_escapeHtml($block['name']); ?></p>
					</div>
				</div>
                <?php
                break;
            ...
            if (!empty($data['followup']->trigger) && !$triggerAdded) {
				...
				<div class="cell grid-x">
					<div class="cell small-8 large-6 acym__summary__card text-center">
						<p class="acym__color__orange"><?php echo acym_escapeHtml(acym_translation('ACYM_MISSING_ADDON')); ?></p>
					</div>
				</div>
                <?php
            }
		...
		<div class="cell grid-x acym__followup__summary__section margin-right-2">
			<h5 class="cell shrink margin-next-1 acym__title acym__title__secondary">
				<b><?php echo acym_escapeHtml(acym_translation('ACYM_CONDITIONS')); ?></b>
			</h5>
			<div class="cell auto acym__followup__summary__modify">
				<a href="<?php echo acym_escapeUrl(acym_completeLink('campaigns&task=edit&step=followupCondition&id='.intval($data['followup']->id))); ?>">
					<i class="acymicon-pencil"></i><span> <?php echo acym_escapeHtml(acym_translation('ACYM_EDIT')); ?></span>
				</a>
			</div>
			<div class="cell grid-x" id="acym__followup__summary__conditions">
                <?php
                if (empty($data['condition'])) {
                    echo acym_escapeHtml(acym_translation('ACYM_NO_CONDITION'));
                } else {
                    echo '<ul class="acym__ul">';
                    foreach ($data['condition'] as $oneCondition) {
                        echo '<li class="cell">'.acym_escapeHtml(ucfirst($oneCondition)).'</li>';
                    }
                    echo '</ul>';
                }
                ?>
		...
		<div class="cell grid-x acym__followup__summary__section margin-right-2 margin-bottom-2">
			<h5 class="cell shrink margin-next-1 acym__title acym__title__secondary">
				<b><?php echo acym_escapeHtml(acym_translation('ACYM_EMAILS')); ?></b>
			</h5>
			<div class="cell auto acym__followup__summary__modify">
				<a href="<?php echo acym_escapeUrl(acym_completeLink('campaigns&task=edit&step=followupEmail&id='.intval($data['followup']->id))); ?>">
					<i class="acymicon-pencil"></i><span> <?php echo acym_escapeHtml(acym_translation('ACYM_EDIT')); ?></span>
				</a>
			</div>
			<div class="cell grid-x acym__listing" id="acym__followup__summary__conditions">
                ...
                if (empty($data['followup']->mails)) {
                    echo acym_escapeHtml(acym_translation('ACYM_NO_EMAIL_FOR_FOLLOWUP'));
                } else {
                    ?>
						...
						<div class="grid-x cell align-middle acym__listing__row">
							<div class="grid-x medium-auto small-11 cell acym__listing__title__container">
								...
								<div class="auto cell grid-x align-center">
                                    <?php
                                    echo acym_escapeHtml($mail->delay_display);
                                    acym_info(['textShownInTooltip' => 'ACYM_DELAY_SUMMARY']);
                                    ?>
								</div>
								<div class="auto cell grid-x align-center acym__followup__summary__listing__action">
									...
									<i class="cell shrink acymicon-delete acym__followup__summary__listing__action-delete margin-left-1"
		...
		<div class="cell grid-x text-center acym__followup__summary__save-button">
			...
			<div class="cell medium-auto grid-x text-right">
				...
				<button data-task="followupActivate" type="submit" class="cell medium-shrink button margin-bottom-0 acy_button_submit">
                    <?php echo acym_escapeHtml(acym_translation('ACYM_ACTIVATE')); ?>
				</button>
			</div>
		</div>
	</div>
