...
<form id="acym_form" action="<?php echo acym_escapeUrl(acym_completeLink(acym_getVar('cmd', 'ctrl'))); ?>" method="post" name="acyForm">
	<div id="acym__followup__trigger"
		...
		<h1 class="margin-top-1 margin-bottom-2 acym__title">
            <?php echo acym_escapeHtml(acym_translation('ACYM_WHAT_TRIGGERS_FOLLOW_UP_SHOULD_START')); ?>
		</h1>
		<div class="cell grid-x grid-margin-x align-center margin-y">
			...
			<div class="acym__selection__card acym__selection__card__disabled cell xxlarge-2 xlarge-3 medium-4 text-center">
				...
				<h1 class="acym__selection__card__title"><?php echo acym_escapeHtml(acym_translation('ACYM_HAVE_SUGGESTION')); ?></h1>
				<p class="acym__selection__card__description"><?php echo acym_escapeHtml(acym_translation('ACYM_HAVE_SUGGESTION_DESC')); ?></p>
				<a href="<?php echo acym_escapeUrl(ACYM_ACYMAILING_WEBSITE); ?>contact/"
				   target="_blank"
				   class="button button-secondary"
				   id="acym__followup__suggest__button"><?php echo acym_escapeHtml(acym_translation('ACYM_SUGGEST_IDEA')); ?></a>
			</div>
		</div>
        <?php
        if (!empty($data['followup']->trigger) && !$oneSelected) {
            echo '<div class="cell grid-x align-center margin-y acym__color__orange"><b>'.acym_escapeHtml(acym_translation('ACYM_MISSING_ADDON')).'</b></div>';
        }
        ?>
