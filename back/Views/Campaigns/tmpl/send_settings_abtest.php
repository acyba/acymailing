...
$abTestParams = empty($data['currentCampaign']->sending_params['abtest']) ? [] : $data['currentCampaign']->sending_params['abtest']; ?>
<h5 class="cell margin-top-1 acym__title acym__title__secondary">
    <?php echo acym_escapeHtml(acym_translation('ACYM_AB_TEST_REPARTITION'));
    acym_info(['textShownInTooltip' => 'ACYM_AB_TEST_REPARTITION_DESC']); ?>
</h5>
<div class="cell grid-x align-center">
	<div class="cell grid-x medium-11 acym__campaign__sendsettings__send-type grid-margin-x margin-top-1">
		<span class="cell shrink"><?php echo acym_escapeHtml(acym_translation('ACYM_AB_TEST_CAMPAIGN_SENT_TO')); ?></span>
	...
	<div class="cell grid-x medium-11 acym__campaign__sendsettings__send-type grid-margin-x margin-top-1 margin-bottom-1">
		<div class="cell">
            <?php
            $days = empty($abTestParams['after']['days']) ? 2 : $abTestParams['after']['days'];
            ?>
			<p class="cell margin-bottom-1">
                <?php echo acym_escapeHtmlWithAllowedTags(
                    acym_translationSprintf(
                        'ACYM_AFTER_X_DAYS',
                        '<input name="sending_params[abtest][after][days]" type="number" min="1" class="intext_input" value="'.acym_escape($days).'">'
                    ),
                    [
                        'input' => [
                            'name' => true,
