...
<h5 class="cell margin-top-1 acym__title acym__title__secondary">
    <?php
    echo acym_escapeHtml(acym_translation('ACYM_WHEN_EMAIL_WILL_BE_SENT'));
    acym_info(['textShownInTooltip' => 'ACYM_PRESELECT_DESC']);
    ?>
</h5>
        ...
        if (empty($data['currentCampaign']->send_specific)) {
            if (!empty($data['currentCampaign']->sent) && empty($data['currentCampaign']->active)) { ?>
				...
				<h3 class="acym__title__primary__color acym__middle_absolute__text text-center">
                    <?php echo acym_escapeHtml(acym_translation('ACYM_CAMPAIGN_ALREADY_QUEUED')); ?>
				</h3>
            <?php } ?>
			<div class="cell grid-x grid-margin-x margin-bottom-1">
				<div class="cell auto grid-x align-center">
                    ...
                    ?>
					<button type="button"
					        class="cell medium-9 small-12 button-radio acym__campaign__sendsettings__buttons-type <?php echo acym_escape($class); ?>"
					        acym-button-radio-group="sendingType"
					        id="acym__campaign__sendsettings__now"
					        data-sending-type="<?php echo acym_escape(CampaignClass::SENDING_TYPE_NOW); ?>">
                        <?php echo acym_escapeHtml(acym_translation('ACYM_NOW')); ?>
					</button>
				</div>
                <?php
				...
				<div class="cell auto grid-x align-center">
                    ...
                    if (!acym_level(ACYM_ESSENTIAL)) {
                        echo 'data-acym-tooltip="'.acym_escape(acym_translationSprintf('ACYM_USE_THIS_FEATURE', 'AcyMailing Essential')).'"';
                    }
                    ?>
					        class="cell medium-9 small-12 button-radio acym__campaign__sendsettings__buttons-type <?php echo acym_escape($class); ?>"
					        acym-button-radio-group="sendingType"
					        id="acym__campaign__sendsettings__scheduled"
					        data-sending-type="<?php echo acym_escape(CampaignClass::SENDING_TYPE_SCHEDULED); ?>">
                        <?php echo acym_escapeHtml(acym_translation('ACYM_SCHEDULED')); ?>
					</button>
				</div>
                <?php
				...
				<div class="cell auto grid-x align-center">
                    ...
                    if (!acym_level(ACYM_ENTERPRISE)) {
                        echo 'data-acym-tooltip="'.acym_escape(acym_translationSprintf('ACYM_USE_THIS_FEATURE', 'AcyMailing Enterprise')).'"';
                    }
                    ?>
					        class="cell medium-9 small-12 button-radio acym__campaign__sendsettings__buttons-type <?php echo acym_escape($class); ?>"
					        acym-button-radio-group="sendingType"
					        id="acym__campaign__sendsettings__auto"
					        data-sending-type="<?php echo acym_escape(CampaignClass::SENDING_TYPE_AUTO); ?>">
                        <?php echo acym_escapeHtml(acym_translation('ACYM_AUTO')); ?>
					</button>
				</div>
			</div>
        <?php } else { ?>
			<div class="cell grid-x margin-bottom-1">
                <?php echo acym_escapeHtmlWithAllowedTags(
                    $data['currentCampaign']->send_specific[0]['whenSettings'],
...
<h5 class="cell margin-top-1 margin-bottom-1 acym__title acym__title__secondary">
    <?php echo acym_escapeHtml(acym_translation('ACYM_ADDITIONAL_SETTINGS')); ?>
</h5>
<div class="cell grid-x margin-left-1">
	<div class="cell medium-11 grid-margin-x grid-x acym__campaign__sendsettings__params margin-left-3"
		...
		<label><?php echo acym_escapeHtml(acym_translation('ACYM_SENT_AS_SOON_CAMPAIGN_SAVE')); ?></label>
	</div>
	<div class="cell grid-x acym__campaign__sendsettings__params margin-left-3"
		...
		<div class="cell grid-x acym__campaign__sendsettings__display-send-type-scheduled">
			<label id="acym__campaign__sendsettings__scheduled__send-date__label" class="cell shrink padding-next-1">
                <?php echo acym_escapeHtml(acym_translation('ACYM_CAMPAIGN_WILL_BE_SENT')); ?>
			</label>
	...
	<div class="cell grid-x align-center margin-left-3 acym__campaign__sendsettings__params"
		...
		<div class="cell grid-x align-center">
			<div class="cell grid-x acym_vcenter">
				<label class="cell"><?php echo acym_escapeHtml(acym_translation('ACYM_THIS_WILL_GENERATE_CAMPAIGN_AUTOMATICALLY')); ?></label>
				<div class="cell grid-x margin-y">
					...
					<div class="cell grid-x medium-5 small-10">
                        ...
                        foreach ($data['triggers_display'] as $key => $display) {
                            echo '<div class="acym__campaign__sendsettings__params__one cell grid-x" data-trigger-show="'.acym_escape($key).'" style="display: none">';
			...
			<div class="cell grid-x margin-top-1">
				<label class="cell">
                    <?php
                    echo acym_escapeHtml(acym_translation('ACYM_NEXT_EXECUTION'));
                    acym_info(['textShownInTooltip' => 'ACYM_NEXT_EXECUTION_DESC']);
                    ?>
				</label>
				<div class="cell medium-5 small-10">
                    <?php
			...
			<div class="cell grid-x margin-top-1">
                ...
                acym_switch([
                    'name' => 'need_confirm',
                    'value' => $data['currentCampaign']->sending_params['need_confirm_to_send'] ?? 1,
                    'label' => acym_translation('ACYM_CONFIRM_AUTOCAMPAIGN'),
                    'labelClass' => 'shrink',
                    'switchContainerClass' => 'shrink',
                    'toggle' => 'acym__campaign__sendsettings__notification',
                ]);
                ?>
			</div>
			<div class="cell grid-x margin-top-1" id="acym__campaign__sendsettings__notification">
				<label class="cell">
                    <?php
                    echo acym_escapeHtml(acym_translation('ACYM_NOTIFICATION_AUTO_CAMPAIGN_GENERATED'));
                    ?>
				</label>
    ...
    if (!empty($data['currentCampaign']->send_specific) && !empty($data['currentCampaign']->send_specific[0]['additionnalSettings'])) {
        echo acym_escapeHtmlWithAllowedTags(
            $data['currentCampaign']->send_specific[0]['additionnalSettings'],
            array_merge_recursive(
                SecurityHelper::ALLOWED_HTML_SELECT,
                [
                    'div' => [
    ...
    if (stripos($trackingMode, 'google') !== false) {
		...
		<div class="cell grid-x align-center" id="utm_settings" <?php echo empty($isTracked) ? 'style="display:none;"' : ''; ?>>
			<h6 class="cell acym__title acym__title__secondary acym__title__secondary__utm">
                <?php echo acym_escapeHtml(acym_translation('ACYM_CUSTOM_UTM')); ?>
			</h6>
			<div class="cell grid-x medium-11 grid-margin-x margin-y">
				...
				<div class="cell medium-5">
					...
					<input name="sending_params[utm_source]" id="utm_source"
					       ...
					       placeholder="newsletter_<?php echo acym_escape($data['currentCampaign']->id); ?>">
				</div>
				<div class="cell medium-5">
				...
				<label class="cell medium-6">
					...
					<input type="text"
					       ...
					       value="<?php echo empty($data['currentCampaign']->sending_params['message_stream_id']) ? ''
                               : acym_escape($data['currentCampaign']->sending_params['message_stream_id']); ?>">
				</label>
			</div>
		</div>
    <?php } ?>
