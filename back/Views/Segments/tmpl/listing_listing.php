			...
			<div data-acy-elementid="<?php echo acym_escape($segment->id); ?>" class="grid-x cell align-middle acym__listing__row">
				...
				<div class="grid-x medium-auto small-11 cell acym__listing__title__container">
					<div class="grid-x large-4 medium-4 small-11 cell acym__listing__title">
						<a class="cell" href="<?php echo acym_escapeUrl(acym_completeLink('segments&task=edit&segmentId='.intval($segment->id))); ?>">
							<h6 class="acym__listing__title__important"><?php echo acym_escapeHtml($segment->name); ?></h6>
						</a>
					</div>
					...
					<div class="cell small-1 acym__listing__controls text-center">
                        ...
                        echo '<i data-acy-table="segment" data-acy-field="active" data-acy-elementid="'.acym_escape($segment->id).'" data-acy-newvalue="'.acym_escape($newvalue).'" class="acym_toggleable '.acym_escape(
                                $class
                            ).'"></i>';
                        ?>
					</div>
					<div class="cell medium-1 hide-for-small-only text-center">
                        <?php echo acym_escapeHtml($segment->id); ?>
					</div>
				</div>
			</div>
        <?php } ?>
