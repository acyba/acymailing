	...
	<div class="cell grid-x margin-top-1">
		...
		<div class="cell grid-x align-justify">
			<div class="cell grid-x large-shrink acym_vcenter">
                ...
                acym_filterStatus($options, $data['status'], 'lists_status');
                ?>
			</div>
	...
	<div class="grid-x acym__listing acym__listing__view__list">
		<div class="grid-x cell acym__listing__header">
			...
			<div class="grid-x medium-auto small-11 cell acym__listing__header__title__container">
				...
				<div class="acym__listing__header__title cell <?php echo acym_escape($columnClassesText); ?> text-center">
                    <?php echo acym_escapeHtml(acym_translation('ACYM_SUBSCRIBED')); ?>
				</div>
				<div class="acym__listing__header__title cell <?php echo acym_escape($columnClassesIcon); ?> text-center">
                    <?php
                    acym_tooltip(
                        [
                ...
                <?php if ($this->config->get('require_confirmation', 1) == 1) { ?>
					<div class="acym__listing__header__title cell show-for-xxlarge <?php echo acym_escape($columnClassesText); ?> text-center">
                        <?php echo acym_escapeHtml(acym_translation('ACYM_NOT_CONFIRMED')); ?>
					</div>
					<div class="acym__listing__header__title cell <?php echo acym_escape($columnClassesIcon); ?> text-center">
                        <?php
                        acym_tooltip(
				...
				<div class="acym__listing__header__title cell <?php echo acym_escape($columnClassesText); ?> text-center">
                    <?php echo acym_escapeHtml(acym_translation('ACYM_INACTIVE')); ?>
				</div>
				<div class="acym__listing__header__title cell <?php echo acym_escape($columnClassesIcon); ?> text-center">
                    <?php
                    acym_tooltip(
                        [
                            'hoveredText' => '<i class="acymicon-close"></i>',
				...
				<div class="grid-x medium-auto small-11 cell acym__listing__title__container">
					<div class="cell auto grid-x acym__listing__title">
						...
						<a class="cell auto" href="<?php echo acym_escapeUrl($linkList); ?>">
                            ...
                            echo '<p class="acym__listing__title__secondary" title="'.acym_escape($list->description).'">'.acym_escapeHtml($list->description).'</p>';
                            ?>
						</a>
					</div>
					<div class="small-1 <?php echo acym_escape($columnWidthClass); ?> text-center small-up-1 cell">
                        <?php
                        acym_tooltip(
                            [
					...
					<div class="large-1 hide-for-small-only hide-for-medium-only cell text-center acym__icon__table">
						<a href="<?php echo acym_escapeUrl($linkList); ?>"><i class="acymicon-pencil" title="<?php echo acym_escape(acym_translation('ACYM_EDIT')); ?>"></i></a>
                        <?php if (acym_isAdmin()) { ?>
					...
					<div class="medium-shrink hide-for-small-only text-center acym__listing__id">
                        <?php echo acym_escapeHtml($list->id); ?>
					</div>
				</div>
			</div>
        <?php } ?>
	</div>
    <?php $data['pagination']->display('lists'); ?>
<?php } ?>
