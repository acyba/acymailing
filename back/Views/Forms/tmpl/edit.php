...
?>
<form id="acym_form" action="<?php echo acym_escapeUrl(acym_completeLink(acym_getVar('cmd', 'ctrl'))); ?>" method="post" name="acyForm" novalidate data-abide>
	...
	<div id="acym__forms" class="grid-x">
		<div class="cell grid-x align-right grid-margin-x margin-y margin-left-0 margin-bottom-1 padding-bottom-0 acym__content">
            <?php acym_cancelButton('ACYM_CANCEL', '', 'button cell medium-6 large-shrink'); ?>
			...
			<button class="cell medium-6 large-shrink button" type="button" @click="save(true)" id="acy__form__save_exit"><?php echo acym_escapeHtml(
                    acym_translation('ACYM_SAVE_EXIT')
                ); ?></button>
		</div>
		<div class="cell">
