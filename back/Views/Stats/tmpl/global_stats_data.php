...
<div class="acym__content acym__stats grid-x cell">
	...
	<div class="cell acym__stats__campaign-choose  margin-bottom-1 large-3 medium-4 small-12">
        <?php if ($data['page_title']) { ?>
			<h2 class="acym__title"><?php echo acym_escapeHtml(acym_translation('ACYM_GLOBAL_STATISTICS')); ?></h2>
        <?php } ?>
	</div>
    ...
    <?php if (!empty($data['selectedMailid']) && !empty($data['lists'])) { ?>
		<div class="cell grid-x margin-bottom-1">
			<h2 class="cell shrink acym__title acym__title__secondary"><?php echo acym_escapeHtml(acym_translation('ACYM_RECEIVER_LISTS')); ?></h2>
			...
			<h2 class="cell acym__title acym__title__secondary">
                <?php echo acym_escapeHtml(acym_translationSprintf('ACYM_NUMBER_OF_RECEIVERS_X', $data['mail']->sent)); ?>
			</h2>
		</div>
    <?php } ?>
    <?php if (empty($data['sentMails'])) { ?>
		<h2 class="acym__listing__empty__title text-center cell margin-bottom-1">
            <?php echo acym_escapeHtml(acym_translation('ACYM_DONT_HAVE_STATS_CAMPAIGN')); ?>
			<a href="<?php echo acym_escapeUrl(acym_completeLink('campaigns&task=edit&step=chooseTemplate')); ?>"><?php echo acym_escapeHtml(
                    acym_translation('ACYM_CREATE_ONE')
                ); ?></a>
		</h2>
    <?php } ?>
	<div class="cell grid-x grid-margin-y" id="acym__stats__export__global__charts__scope">
		<div class="cell grid-x acym__content acym__stats__donut__chart align-center">
			<h2 class="cell acym__title acym__title__secondary"><?php echo acym_escapeHtml(acym_translation('ACYM_EMAIL_STATISTICS')); ?></h2>
            <?php if (empty($data['sentMails'])) { ?>
				<h4 class="cell acym__subtitle__stats text-center"><b><?php echo acym_escapeHtml(acym_translation('ACYM_LOOK_AT_THESE_AMAZING_DONUTS')); ?></b></h4>
            <?php } ?>
		...
		<div class="cell grid-x acym__content acym__stats__pie__chart">
			<h2 class="cell acym__title acym__title__secondary"><?php echo acym_escapeHtml(acym_translation('ACYM_OPENING_PLATFORMS')); ?></h2>
            <?php if (empty($data['devices'])) { ?>
				<h4 class="cell acym__subtitle__stats text-center"><b><?php echo acym_escapeHtml(acym_translation('ACYM_EMAIL_NOT_OPEN_EXAMPLE_STATS')); ?></b></h4>
				...
				<div class="acym__stats__pie__one-chart cell large-3 medium-6">
                    <?php acym_pieChart($data['example_devices_chart'], '', acym_translation('ACYM_DEVICES')); ?>
				</div>
				<div class="acym__stats__pie__one-chart cell large-3 medium-6">
                    <?php acym_pieChart($data['example_source_chart'], '', acym_translation('ACYM_OPENED_WITH')); ?>
				</div>
				<div class="cell large-3 hide-for-small-only hide-for-medium-only"></div>
            <?php } else { ?>
				<div class="cell large-3 hide-for-small-only hide-for-medium-only"></div>
				<div class="acym__stats__pie__one-chart cell large-3 medium-6">
                    <?php acym_pieChart($data['devices'], 'devices', acym_translation('ACYM_DEVICES')); ?>
				</div>
				<div class="acym__stats__pie__one-chart cell large-3 medium-6">
                    <?php acym_pieChart($data['openedWith'], 'opened_with', acym_translation('ACYM_OPENED_WITH')); ?>
				</div>
				<div class="cell large-3 hide-for-small-only hide-for-medium-only"></div>
            <?php } ?>
		</div>
		<div class="cell acym__content grid-x">
			<h2 class="cell shrink acym__title acym__title__secondary">
                <?php echo acym_escapeHtml(acym_translation('ACYM_OPEN_TIME_CHART'));
                acym_info(['textShownInTooltip' => 'ACYM_OPEN_TIME_CHART_DESC']); ?>
			</h2>
		...
		<div class="cell grid-x acym__content acym__stats__chart__line">
			<h2 class="cell acym__title acym__title__secondary"><?php echo acym_escapeHtml(acym_translation('ACYM_OPEN_CLICK_RATE')); ?></h2>
            <?php if (!$data['mail']->hasStats) { ?>
				<h4 class="acym__subtitle__stats cell text-center">
					<b><?php echo acym_escapeHtml(acym_translation('ACYM_NO_OPEN_CLICK_DATA_YET')); ?></b>
					<br>
                    <?php echo acym_escapeHtml(acym_translation('ACYM_HERE_AN_EXEMPLE_OF_WHAT_YOU_CAN_GET')); ?>
				</h4>
                <?php
                acym_lineChart(
                    $data['example_line_chart']['month'],
                    $data['example_line_chart']['day'],
                    $data['example_line_chart']['hour'],
                );
            } else {
                if ($data['show_date_filters']) {
					...
					<div class="cell grid-x margin-bottom-1">
						...
						<label class="cell grid-x large-3 medium-6 small-12 acym__stats__chart__date ">
							<p class="cell shrink"><?php echo acym_escapeHtml(acym_translation('ACYM_START')); ?></p>
							<input
						...
						<label class="cell grid-x large-3 medium-6 small-12 acym__stats__chart__date ">
							<p class="cell shrink"><?php echo acym_escapeHtml(acym_translation('ACYM_END')); ?></p>
							<input
									class="acy_date_picker auto cell text-center acym__stats__chart__line__input__date"
									id="chart__line__end"
									type="text"
				...
				<div id="acym__stats__chart__line__canvas" class="cell">
                    <?php acym_lineChart($data['mail']->month, $data['mail']->day, $data['mail']->hour); ?>
				</div>
            <?php } ?>
		</div>
	</div>
