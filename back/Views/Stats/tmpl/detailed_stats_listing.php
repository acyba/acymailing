...
?>
	<div class="acym__content acym__stats" id="acym_stats_detailed">
        ...
        if (empty($data['selectedMailid'])) { ?>
			<h1 class="acym__listing__empty__title text-center cell"><?php echo acym_escapeHtml(acym_translation('ACYM_SELECT_AN_EMAIL_FIRST')); ?></h1>
        <?php } elseif (!empty($data['emptyDetailed']) && $data['emptyDetailed'] == 'campaigns') { ?>
			<h1 class="acym__listing__empty__title text-center cell">
				...
				<a href="<?php echo acym_escapeUrl(acym_completeLink('campaigns&task=edit&step=chooseTemplate')); ?>">
                    <?php echo acym_escapeHtml(acym_translation('ACYM_CREATE_ONE')); ?>
				</a>
			</h1>
        <?php } elseif (!empty($data['emptyDetailed']) && $data['emptyDetailed'] == 'stats') { ?>
			<h1 class="acym__listing__empty__title text-center cell"><?php echo acym_escapeHtml(acym_translation('ACYM_DONT_HAVE_STATS_THIS_CAMPAIGN')); ?></h1>
				...
				<div class="cell grid-x auto">
					<div class="cell grid-x auto">
						<div class="medium-5 small-12 cell acym_stats_detailed_search">
                            <?php acym_filterSearch($data['search'], 'detailed_stats_search', 'ACYM_SEARCH'); ?>
						</div>
        ...
        <?php } else { ?>
				...
				<div class="grid-x cell acym__listing__header">
					<div class="grid-x medium-auto small-11 cell">
						...
						<div class="large-2 medium-3 small-4 cell acym__listing__header__title">
                            <?php echo acym_escapeHtml(acym_translation('ACYM_SUBSCRIBER')); ?>
						</div>
						...
						<div class="large-1 medium-1 small-1 cell acym__listing__header__title text-center">
                            <?php echo acym_escapeHtml(acym_translation('ACYM_OPENED')); ?>
						</div>
						...
						<div class="large-1 medium-1 small-1 cell acym__listing__header__title text-center">
                            <?php echo acym_escapeHtml(acym_translation('ACYM_TOTAL_CLICK')); ?>
						</div>
						<div class="large-1 hide-for-small-only hide-for-medium-only cell acym__listing__header__title text-center">
                            <?php echo acym_escapeHtml(acym_translation('ACYM_UNSUBSCRIBED')); ?>
						</div>
						<div class="large-1 hide-for-small-only hide-for-medium-only cell acym__listing__header__title text-center">
                            <?php echo acym_escapeHtml(acym_translation('ACYM_BOUNCED')); ?>
						</div>
                        ...
                        <?php } ?>
						<div class="large-1 medium-1 small-1 cell acym__listing__header__title text-center">
                            <?php echo acym_escapeHtml(acym_translation('ACYM_SENT')); ?>
						</div>
					</div>
				</div>
                <?php
					...
					<div class="grid-x cell align-middle acym__listing__row">
						<div class="grid-x medium-auto small-11 cell">
							...
							<div class="large-2 medium-3 small-4 cell acym__listing__detailed__stats__content">
                                <?php if (acym_isAllowed('users')) { ?>
									<a href="<?php echo acym_escapeUrl(acym_completeLink('users&task=edit&userId='.$detailed_stat->user_id)); ?>"
									   class="acym__color__blue word-break"><?php echo acym_escapeHtml($detailed_stat->email); ?></a>
                                <?php } else { ?>
                                    <?php echo acym_escapeHtml($detailed_stat->email); ?>
                                <?php } ?>
							</div>
							<div class="xlarge-1 hide-for-large-only hide-for-medium-only hide-for-small-only cell acym__listing__header__title">
                                <?php if (acym_isAllowed('users')) { ?>
							...
							<div class="large-1 medium-1 small-1 cell acym__listing__detailed__stats__content text-center">
								<p class="hide-for-medium-only hide-for-small-only"><?php echo acym_escapeHtml($detailed_stat->open); ?></p>
							</div>
							...
							<div class="large-1 medium-1 small-1 cell acym__listing__detailed__stats__content text-center">
								<p class="hide-for-medium-only hide-for-small-only"><?php echo empty($detailed_stat->total_click)
                                        ? 0
                                        : acym_escapeHtml(
                                            $detailed_stat->total_click
                                        ); ?></p>
							</div>
							<div class="large-1 hide-for-small-only hide-for-medium-only cell acym__listing__detailed__stats__content text-center">
                                <?php echo acym_escapeHtml($detailed_stat->unsubscribe); ?>
							</div>
							<div class="large-1 hide-for-small-only hide-for-medium-only cell acym__listing__detailed__stats__content text-center">
                                <?php
                                if (empty($detailed_stat->bounce)) {
                                    echo acym_escapeHtml($detailed_stat->bounce);
                                } else {
                                    acym_tooltip(
                                        [
                            ...
                            <?php if (acym_isTrackingSalesActive()) { ?>
								<div class="large-1 hide-for-small-only hide-for-medium-only cell acym__listing__detailed__stats__content text-center">
                                    ...
                                    if (!empty($detailed_stat->sales) && !empty($detailed_stat->currency)) {
                                        echo acym_escapeHtml(round($detailed_stat->sales, 2).' '.$detailed_stat->currency);
                                    } else {
                                        echo '-';
                                    }
                                    ?>
            ...
            <?php
            $data['pagination']->display('detailed_stats');
        } ?>
	</div>
<?php
acym_formOptions();
