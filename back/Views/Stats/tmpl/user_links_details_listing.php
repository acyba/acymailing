...
?>
	<div class="acym__content acym__stats" id="acym_stats_user_links_details">
				...
				<div class="cell grid-x auto">
					<div class="cell grid-x auto">
						<div class="medium-5 small-12 cell acym_stats_detailed_search">
                            <?php acym_filterSearch($data['search'], 'user_links_details_search', 'ACYM_SEARCH'); ?>
						</div>
        ...
        <?php } else { ?>
				...
				<div class="cell grid-x">
					...
					<div class="cell auto align-right grid-x">
						<button type="button"
						        ...
						        data-task="exportUserLinksDetails"><?php echo acym_escapeHtml(
                                acym_translation(
                                    'ACYM_EXPORT'
                                )
                            ); ?></button>
				...
				<div class="grid-x cell acym__listing__header">
					<div class="grid-x medium-auto small-11 cell">
						...
						<div class="large-2 medium-3 cell acym__listing__header__title text-center hide-for-small-only acym__listing__user_links_details__click-date">
                            <?php echo acym_escapeHtml(acym_translation('ACYM_CLICK_DATE')); ?>
						</div>
					...
					<div class="grid-x cell align-middle acym__listing__row">
						<div class="grid-x medium-auto small-11 cell">
							<div class="medium-auto small-12 cell acym__listing__user_links_details__content acym_text_ellipsis">
                                ...
                                echo '<a href="'.acym_escapeUrl($link).'">'.acym_escapeHtml($userLinkDetails->email.' - '.$userLinkDetails->user_name).'</a>';
                                ?>
							</div>
							<div class="medium-auto small-12 cell acym__listing__user_links_details__content acym__stats__link">
                                <?php
                                echo acym_escapeHtml($userLinkDetails->url_name).'<a href="'.acym_escapeUrl(
                                        $userLinkDetails->url_name
                                    ).'" class="acym__listing__user_links_details__content__links" target="_blank"><i class="acymicon-external-link"></i></a>';
                                ?>
							</div>
							<div class="medium-auto small-12 cell acym__listing__user_links_details__content">
							<span data-acym-tooltip="<?php echo acym_escape(acym_translationSprintf('ACYM_MAIL_NAME_X', $userLinkDetails->mail_name)); ?>">
								<?php echo acym_escapeHtml($userLinkDetails->mail_subject); ?>
							</span>
							</div>
							<div class="large-2 medium-3 hide-for-small-only cell acym__listing__user_links_details__content acym__listing__user_links_details__click-date text-center">
                                <?php echo empty($userLinkDetails->date_click)
                                    ? '-'
                                    : acym_escapeHtml(
                                        acym_date(acym_getTimeFromUTCDate($userLinkDetails->date_click), acym_getDateTimeFormat())
                                    ); ?>
							</div>
							<div class="large-2 medium-3 small-4 cell acym__listing__user_links_details__content text-center">
                                <?php echo empty($userLinkDetails->click) ? 0 : acym_escapeHtml($userLinkDetails->click); ?>
							</div>
						</div>
					</div>
                    <?php
            ...
            <?php
            $data['pagination']->display('user_links_details');
        } ?>
	</div>
<?php
acym_formOptions();
