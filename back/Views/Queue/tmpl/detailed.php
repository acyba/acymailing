...
<form id="acym_form" action="<?php echo acym_escapeUrl(acym_completeLink(acym_getVar('cmd', 'ctrl'))); ?>" method="post" name="acyForm" data-abide novalidate>
	...
	<div id="acym__queue" class="acym__content">
        ...
        $workflow->displayTabs($this->steps, 'detailed');
        ?>
        ...
        <?php } else { ?>
            ...
            <?php } else { ?>
				<div class="grid-x acym__listing acym__listing__view__dqueue">
					<div class="cell grid-x acym__listing__header hide-for-small-only">
						<div class="acym__listing__header__title cell large-2 medium-3">
                            <?php echo acym_escapeHtml(acym_translation('ACYM_SENDING_DATE')); ?>
						</div>
						...
						<div class="acym__listing__header__title cell medium-1 hide-for-small-only text-center">
                            <?php echo acym_escapeHtml(acym_translation('ACYM_TRY')); ?>
						</div>
						<div class="acym__listing__header__title cell medium-1 text-center">
                            <?php echo acym_escapeHtml(acym_translation('ACYM_DELETE')); ?>
						</div>
					</div>
                    <?php foreach ($data['allElements'] as $row) { ?>
						<div data-acy-elementid="<?php echo acym_escape($row->id.'_'.$row->user_id); ?>" class="cell grid-x acym__listing__row">
							...
							<div class="cell medium-4 acym__listing__title">
								<div class="acym__listing__title acym_text_ellipsis"><?php echo acym_escapeHtml($row->name); ?></div>
							...
							<div class="cell large-4 medium-3">
								<a href="<?php echo acym_escapeUrl(acym_completeLink('users&task=edit&userId='.$row->user_id)); ?>">
                                    <?php
                                    if (empty($row->user_name)) {
                                        echo acym_escapeHtml($row->email);
                                    } else {
                                        echo '<span class="hide-for-medium-only hide-for-small-only">'.acym_escapeHtml($row->user_name).' (</span>'.acym_escapeHtml(
                                                $row->email
                                            ).'<span class="hide-for-medium-only hide-for-small-only">)</span>';
                                    }
                                    ?>
								</a>
							</div>
							<div class="cell medium-1 hide-for-small-only text-center">
                                <?php echo acym_escapeHtml($row->try); ?>
							</div>
                ...
                <?php $data['pagination']->display('dqueue'); ?>
            <?php } ?>
        <?php } ?>
