	...
	<div id="acym__editor__content" class="grid-x acym__content acym__editor__area">
		<div class="cell grid-x align-right grid-margin-x margin-bottom-0 margin-y">
            ...
            <?php if (!empty($data['list_id'])) {
                echo '<input type="hidden" id="acym__mail__list-id" name="mail[list_id]" value="'.acym_escape($data['list_id'][0]).'">';
            } ?>
		...
		<div class="cell grid-x grid-padding-x acym__editor__content__options margin-y">
            ...
            if (!empty($data['return'])) {
                echo '<input type="hidden" name="return" value="'.acym_escape($data['return']).'"/>';
            }
            if (!empty($data['fromId'])) {
                echo '<input type="hidden" name="fromId" value="'.acym_escape($data['fromId']).'"/>';
            }
            ...
            } elseif (in_array($data['mail']->type, [$data['mailClass']::TYPE_AUTOMATION, $data['mailClass']::TYPE_OVERRIDE, $data['mailClass']::TYPE_SCENARIO])) {
