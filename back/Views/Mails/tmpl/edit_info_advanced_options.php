...
<div class="cell grid-x acym__toggle__arrow">
	<p class="cell medium-shrink acym__toggle__arrow__trigger"><?php echo acym_escapeHtml(acym_translation('ACYM_ADVANCED_OPTIONS')); ?>
		<i class="acymicon-keyboard-arrow-down"></i></p>
	<div class="cell acym__toggle__arrow__contain">
		<div class="grid-x grid-padding-x margin-y">
            <?php
            if (
                !empty($data['mail']->type)
                && in_array(
                    $data['mail']->type,
                    [
                        MailClass::TYPE_WELCOME,
                        MailClass::TYPE_TEMPLATE,
                        MailClass::TYPE_UNSUBSCRIBE,
                        MailClass::TYPE_NOTIFICATION,
                    ]
                )
            ) {
                ?>
            ...
            <?php } ?>
            <?php if (!empty($data['mail']->type) && $data['mail']->type === MailClass::TYPE_TEMPLATE) { ?>
				<div class="cell grid-x">
					<div class="cell grid-x medium-6">
						<label for="acym__mail__edit__thumbnail" class="cell shrink">
                            <?php
                            echo acym_escapeHtml(acym_translation('ACYM_EMAIL_CUSTOM_THUMBNAIL'));
                            acym_info(['textShownInTooltip' => 'ACYM_EMAIL_CUSTOM_THUMBNAIL_DESC']);
                            ?>
						</label>
                        ...
                        <?php if (!empty($data['mail']->thumbnail) && strpos($data['mail']->thumbnail, '_custom_') !== false) { ?>
							<div id="acym__mail__edit__thumbnail--saved" class="cell shrink grid-x">
								<span id="acym__mail__edit__thumbnail--file-saved" class="cell shrink acym_vcenter margin-left-1">
									<?php echo acym_escapeHtml($data['mail']->thumbnail); ?>
								</span>
            ...
            <?php if (empty($data['mail']->drag_editor)) { ?>
				<div class="cell grid-x medium-6" id="acym__mail__edit__html__stylesheet__container">
					...
					<textarea
							...
							cols="30"
							rows="15"
							type="text"><?php echo acym_escapeHtml($stylesheet); ?></textarea>
				</div>
            <?php } ?>
			<div class="cell medium-auto">
				<label for="acym__mail__edit__custom__header"><?php
                    echo acym_escapeHtml(acym_translation('ACYM_CUSTOM_HEADERS'));
                    acym_info(['textShownInTooltip' => 'ACYM_EMAIL_CUSTOM_HEADERS_DESC']);
                    ?>
				</label>
				<textarea id="acym__mail__edit__custom__header" name="editor_headers" cols="30" rows="15" type="text"><?php echo acym_escapeHtml(
                        $data['mail']->headers ?? ''
                    ); ?></textarea>
			</div>
            <?php if (!empty($data['mail']->type) && !in_array($data['mail']->type, [MailClass::TYPE_STANDARD, MailClass::TYPE_TEMPLATE])) { ?>
				<div class="cell grid-x">
					<div class="cell grid-x medium-6">
