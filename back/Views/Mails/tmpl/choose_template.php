	...
	<div class="grid-x grid-margin-x margin-top-3">
        ...
        if (empty($data['allMails']) && empty($data['search']) && empty($data['tag']) && empty($data['status'])) { ?>
			<div class="grid-x cell text-center">
				<h1 class="acym__listing__empty__title cell"><?php echo acym_escapeHtml(acym_translation('ACYM_YOU_DONT_HAVE_ANY_TEMPLATE')); ?></h1>
				...
				<div class="cell medium-4">
					<a href="<?php echo acym_escapeUrl(
					   ...
					   id="acym__templates__choose__create__empty">
                        <?php echo acym_escapeHtml(acym_translation('ACYM_START_FROM_EMPTY_TEMPLATE')); ?>
					</a>
				</div>
			</div>
        <?php } else { ?>
			<div class="cell grid-x margin-bottom-2 grid-margin-x">
				<div class="medium-auto cell">
                    <?php acym_filterSearch($data['search'], 'mailchoose_search', 'ACYM_SEARCH'); ?>
				</div>
				...
				<div class="grid-x medium-shrink text-center cell acym__templates__choose__type-templates">
                    ...
                    ?>
					<a href="<?php echo acym_escapeUrl($link); ?>"
					   class="button"
					   id="acym__templates__choose__create__empty">
                        <?php echo acym_escapeHtml(acym_translation('ACYM_START_FROM_EMPTY_TEMPLATE')); ?>
					</a>
                    ...
                    foreach ($data['allMails'] as $oneTemplate) {
						...
						<div class="cell grid-x acym__templates__oneTpl acym__listing__block">
							...
							<input type="hidden"
							       ...
							       value="<?php echo acym_escapeUrl($link); ?>" />
							<div class="cell acym__templates__pic text-center">
                                ...
                                if ($oneTemplate->drag_editor) {
                                    echo '<div class="acym__templates__choose__ribbon acyeditor">'.acym_escapeHtml(acym_translation('ACYM_DD_EDITOR')).'</div>';
                                } else {
                                    echo '<div class="acym__templates__choose__ribbon htmleditor">'.acym_escapeHtml(acym_translation('ACYM_HTML_EDITOR')).'</div>';
                                }
                                ?>
							</div>
							<div class="cell grid-x acym__templates__footer text-center">
								...
								<div class="cell"><?php echo acym_escapeHtml(acym_date($oneTemplate->creation_date, acym_getDateTimeFormat('ACYM_DATE_FORMAT_LC3'))); ?></div>
							</div>
						</div>
                    <?php } ?>
				</div>
                <?php $data['pagination']->display('mailchoose'); ?>
            <?php } ?>
        <?php } ?>
	</div>
<?php
