...
<form id="acym_form" action="<?php echo acym_escapeUrl(acym_completeLink(acym_getVar('cmd', 'ctrl'))); ?>" method="post" name="acyForm">
	...
	<div id="acym__fields" class="acym__content">
		<div class="cell acym__listing__actions margin-bottom-1 grid-x">
            ...
            acym_listingActions($actions, acym_translation('ACYM_DELETE_CUSTOM_FIELDS_TYPE_DATE'));
            ?>
		</div>
			...
			<div class="grid-x cell acym__listing__header">
				...
				<div class="grid-x medium-auto small-8 cell">
					...
					<div class="medium-auto hide-for-small-only cell acym__listing__header__title ">
                        <?php echo acym_escapeHtml(acym_translation('ACYM_FIELD_TYPE')); ?>
					</div>
					<div class="medium-1 small-3 small-text-right text-center cell acym__listing__header__title">
                        <?php echo acym_escapeHtml(acym_translation('ACYM_REQUIRED')); ?>
					</div>
					...
					<div class="medium-1 small-2 text-center cell acym__listing__header__title">
                        <?php echo acym_escapeHtml(acym_translationSprintf('ACYM_ID')); ?>
					</div>
				</div>
			</div>
					...
					<div class="grid-x cell align-middle acym__listing__row" data-id-element="<?php echo acym_escape($field->id); ?>">
						...
						<div class="grid-x medium-auto small-8 cell acym__field__listing">
							<div class="medium-4 small-7 cell acym__listing__title grid-x">
								<a href="<?php echo acym_escapeUrl(acym_completeLink('fields&task=edit&fieldId='.$field->id)); ?>" class="cell auto">
									<h6><?php echo acym_escapeHtml(acym_translation($field->name)); ?></h6>
								</a>
							</div>
							<div class="medium-auto hide-for-small-only cell acym__listing__title ">
								<h6><?php echo acym_escapeHtml(acym_translation('ACYM_'.strtoupper(acym_escape($field->type)))); ?></h6>
							</div>
							<div class="acym__listing__controls acym__field__controls medium-1 small-3 text-center cell">
                                <?php
                                $class = in_array($field->id, [2, $data['languageFieldId']]) ? 'acym_opacity-5 acym_disabled' : 'acym_toggleable cursor-pointer';
                                $class .= $field->required == 1 ? ' acymicon-check-circle acym__color__green' : ' acymicon-times-circle acym__color__red';
                                echo '<i data-acy-table="field" 
										data-acy-field="required" 
										data-acy-elementid="'.acym_escape($field->id).'" 
										class="'.acym_escape($class).'"';
                                echo 'data-acy-newvalue="'.intval($field->required == 1 ? 0 : 1).'"';
                                echo '></i>';
                                ?>
							</div>
							<div class="acym__listing__controls hide-for-small-only acym__field__controls medium-1 small-1 text-center cell">
                                <?php
                                $class = in_array($field->id, [1, 2, $data['languageFieldId']]) ? 'acym_opacity-5 acym_disabled' : 'acym_toggleable cursor-pointer';
                                $class .= $field->active == 1 ? ' acymicon-check-circle acym__color__green' : ' acymicon-times-circle acym__color__red';
                                echo '<i data-acy-table="field" 
										data-acy-field="active" 
										data-acy-elementid="'.acym_escape($field->id).'" 
										class="'.acym_escape($class).'"';
                                echo 'data-acy-newvalue="'.intval($field->active == 1 ? 0 : 1).'"';
                                echo '></i>';
                                ?>
							</div>
							<h6 class="text-center medium-1 small-2 acym__listing__text"><?php echo acym_escapeHtml($field->id); ?></h6>
						</div>
					</div>
                <?php } ?>
			</div>
