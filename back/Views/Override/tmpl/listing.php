...
<form id="acym_form" action="<?php echo acym_escapeUrl(acym_completeLink(acym_getVar('cmd', 'ctrl'))); ?>" method="post" name="acyForm">
	...
	<div id="acym__override" class="acym__content cell">
        ...
        $workflow->displayTabs($this->tabs, 'listing&overrideMailSource='.$data['source']);
        ?>
		<div class="cell grid-x margin-top-1">
			...
			<div class="cell grid-x align-justify">
				<div class="cell grid-x large-shrink acym_vcenter">
                    ...
                    acym_filterStatus($options, $data['status'], 'emails_override_status');
                    ?>
				</div>
					...
					<div class="grid-x medium-auto small-11 cell acym__listing__title__container">
						<div class="cell medium-4 acym__listing__title">
							<a href="<?php echo acym_escapeUrl(
                                acym_completeLink('mails&task=edit&type=override&id='.intval($override->mail_id).'&return='.urlencode(acym_currentURL()))
                            ); ?>">
								<p><?php
                                    $subject = preg_replace('#^{trans:([A-Z_]+)(|.+)*}$#', '$1', $override->subject);
                                    $subject = str_replace('%', '%%', $subject);
                                    echo acym_escapeHtml(acym_translationSprintf($subject, '{param1}', '{param2}'));
                                    ?></p>
							</a>
						</div>
						<div class="cell medium-6">
                            <?php echo acym_escapeHtml(acym_translation($override->description)); ?>
						</div>
        ...
        <?php $data['pagination']->display('override'); ?>
