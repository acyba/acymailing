	...
	<form id="acym_form"
		...
		<div id="acym__plugin__available__application" class="cell grid-x">
			<div class="cell grid-x grid-margin-x margin-bottom-2">
				<input type="text" class="cell medium-3" v-model="search" placeholder="<?php echo acym_escape(acym_translation('ACYM_SEARCH')); ?>">
					...
					<div class="acym__plugins__card cell xlarge-3 large-4 medium-6 grid-y align-justify" v-for="(plugin, index) in displayedPlugins">
						...
						<div class="cell grid-x acym__plugins__card__actions">
							<div v-if="!installed[plugin.file_name]" v-show="!rightLevel(plugin.level)" class="cell grid-x align-center">
								<a :data-acym-tooltip="<?php echo acym_escape(json_encode(acym_translation('ACYM_YOU_DONT_HAVE_THE_RIGHT_LEVEL'))); ?> + ucfirst(plugin.level)"
								   target="_blank"
								   href="<?php echo acym_escapeUrl(ACYM_ACYMAILING_WEBSITE); ?>pricing"
								   class="acym__plugins__button cell small-5 acym__plugins__button__purchase text-center button button-primary">
                                    <?php echo acym_escapeHtml(acym_translation('ACYM_PURCHASE')); ?>
							...
							<div v-if="!installed[plugin.file_name]" v-show="rightLevel(plugin.level)" class="cell grid-x acym__plugins__card__actions margin-top-1">
								<button v-show="'<?php echo acym_escape(ACYM_CMS); ?>' == 'joomla'"
								        type="button"
										...
										<span v-show="!downloading[plugin.file_name]">
											<?php echo acym_escapeHtml(acym_translation('ACYM_DOWNLOAD')); ?>
											<i class="acymicon-download"></i>
										</span>
									<span v-show="downloading[plugin.file_name]"><?php acym_loaderLogo(); ?></span>
								</button>
								<a v-show="'<?php echo acym_escape(ACYM_CMS); ?>' == 'wordpress'"
								   target="_blank"
							...
							<div v-if="installed[plugin.file_name]" class="cell grid-x acym__plugins__card__actions margin-top-1">
								<button type="button" class="acym__plugins__button cell text-center acym__plugins__button-disabled button button-secondary">
                                    <?php echo acym_escapeHtml(acym_translation('ACYM_ADD_ON_SUCCESSFULLY_INSTALLED')); ?>
