...
<div class="cell grid-x acym__content" id="acym__plugin__installed">
	<form id="acym_form"
        ...
        $workflow->display($data['tabs'], $data['tab'], false, true);
        ?>
		<div id="acym__plugin__installed__application" class="cell grid-x">
            <?php
            if (empty($data['plugins'])) { ?>
				<div class="cell grid-x align-center">
					<h2 class="cell text-center acym__title__primary__color"><?php echo acym_escapeHtml(acym_translation('ACYM_YOU_DONT_HAVE_ADD_ONS')); ?></h2>
					<a href="<?php echo acym_escapeUrl(acym_completeLink('plugins&task=available')); ?>"
					   class="cell shrink button text-center margin-top-1 margin-bottom-2"><?php echo acym_escapeHtml(acym_translation('ACYM_DOWNLOAD_MY_FIRST_ONE')); ?></a>
				</div>
            <?php } else { ?>
				<div class="cell grid-x grid-margin-x margin-bottom-1">
					<input type="text" class="cell medium-3 margin-bottom-0" v-model="search" placeholder="<?php echo acym_escape(acym_translation('ACYM_SEARCH')); ?>">
					...
					<div class="cell grid-x medium-5 align-right">
						...
						<button type="button" class="acy_button_submit button button-secondary acym_vcenter" data-task="checkUpdates">
							<i class="acymicon-autorenew"></i><?php echo acym_escapeHtml(acym_translation('ACYM_CHECK_FOR_UPDATES')); ?>
						</button>
					</div>
				</div>
				<div class="cell grid-x" v-show="noPluginTodisplay" style="display: none;">
					<h2 class="cell text-center acym__title__primary__color"><?php echo acym_escapeHtml(acym_translation('ACYM_NO_ADD_ONS_TO_DISPLAY')); ?></h2>
				</div>
								...
								<div class="acym__plugins__card__params">
									<div class="acym__plugins__card__params__title-line">
										...
										<div class="acym__plugins__card__params__icons">
											<button v-show="plugin.uptodate == '0'" data-acym-tooltip="<?php echo acym_escape(acym_translation('ACYM_UPDATE')); ?>"
											        type="button"
												...
												<span v-show="updating[plugin.id]"><?php acym_loaderLogo(); ?></span>
											</button>
											...
											<div v-if="plugin.settings && plugin.settings == 'not_installed'" class="acym__icon">
												<i class="acymicon-cog cell shrink acym__plugins__settings__toggle__blocked cursor-pointer acym__color__medium-gray acym__tooltip">
													<span class="acym__tooltip__text"><?php echo acym_escapeHtml(
                                                            acym_translation('ACYM_SETTINGS_AVAILABLE_INSTALLED_EXTENSION')
                                                        ); ?></span>
												</i>
											</div>
									...
									<div v-if="plugin.type == 'ADDON'" class="acym__plugins__card__actions cell grid-x acym_vcenter" v-show="rightLevel(plugin.level)">
										<div class="cell grid-x acym_vcenter medium-8">
											<span class="cell shrink"><?php echo acym_escapeHtml(acym_translation('ACYM_ENABLED')); ?>:</span>
							...
							<div class="cell grid-x acym__plugins__settings__container" v-if="plugin.settings">
								...
								<div class="cell grid-x align-center">
									<button type="button"
									        ...
									        class="cell medium-5 button acym__button__cancel"><?php echo acym_escapeHtml(acym_translation('ACYM_CANCEL')); ?></button>
