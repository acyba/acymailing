	...
	<div class="cell grid-x text-center">
		<h6 class="cell">
            <?php echo acym_escapeHtmlWithAllowedTags(
                acym_translation('ACYM_CHOOSE_FILE_WITH_USER_DATA'),
                [
                    'b' => [],
                ]
            ); ?>
		</h6>
		<p class="acym__color__dark-gray cell text-center"><?php echo acym_escapeHtml(
                acym_translation('ACYM_IMPORT_USER_FROM_FILE_INFORMATION_MESSAGE_BELOW_CHOOSE_FILE_BUTTON')
            ); ?></p>
		...
		<div class="acym__users__import__from_file__file cell grid-x" style="display: none;">
			...
			<div class="cell medium-shrink margin-top-2">
				...
				<i title="<?php echo acym_escape(acym_translation('ACYM_DELETE')); ?>" class="acymicon-close acym__users__import__from_file__file__close  acym__color__red"></i>
			</div>
			<div class="cell medium-auto"></div>
		</div>
		...
		<div class="cell grid-x grid-margin-x margin-top-2">
			...
			<button class="cell button medium-shrink margin-bottom-0 acym__users__import__from_file__button-valid" disabled>
                <?php echo acym_escapeHtml(acym_translation('ACYM_IMPORT_THIS_FILE')); ?>
			</button>
			<div class="cell hide-for-small-only medium-auto"></div>
		</div>
	</div>
