...
<div id="acym__users__import__from_database" class="grid-x padding-vertical-2 padding-horizontal-2">
	...
	<div class="cell large-6 grid-x margin-y">
		<label class="cell medium-4" for="acym__users__import__from_database__field--tablename"><?php echo acym_escapeHtml(acym_translation('ACYM_TABLE_NAME')); ?></label>
        <?php
        ...
        if (!empty($userFields)) {
            foreach ($userFields as $oneUserField) {
                if (!in_array($oneUserField, ['id', 'key', 'automation'])) {
                    echo '<label class="cell medium-4" for="acym__users__import__from_database__field--'.acym_escape($oneUserField).'">'.acym_escapeHtml($oneUserField).'</label>';
                    echo '<div class="cell medium-8">';
                    echo '<select acym-data-infinite class="acym__users__import__from_database__fields acym__select" name="fields['.acym_escape(
                            $oneUserField
                        ).']" id="acym__users__import__from_database__field--'.acym_escape($oneUserField).'"></select>';
                    echo '</div>';
                }
            }
        }
        if ($this->config->get('require_confirmation')) { ?>
			<div class="cell grid-x">
                <?php acym_switch(['name' => 'import_confirmed_database', 'value' => 1, 'label' => acym_translation('ACYM_IMPORT_USERS_AS_CONFIRMED')]); ?>
			</div>
        <?php } ?>
