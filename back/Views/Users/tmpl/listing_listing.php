		...
		<div class="grid-x cell acym__listing__header">
			...
			<div class="grid-x medium-auto small-11 cell acym__listing__header__title__container">
				...
				<div class="cell hide-for-small-only hide-for-medium-only large-2 xlarge-1 acym__listing__header__title">
                    <?php echo acym_escapeHtml(acym_translation('ACYM_DATE_CREATED')); ?>
				</div>
                <?php
                if (!empty($data['fields'])) {
                    foreach ($data['fields'] as $field) {
						...
						<div class="cell medium-auto hide-for-small-only acym__listing__header__title">
                            <?php echo acym_escapeHtml(acym_translation($field)); ?>
						</div>
                        <?php
                    }
                }
                ...
                <?php if (acym_isAdmin()) { ?>
					<div class="cell medium-1 hide-for-small-only acym__listing__header__title">
                        <?php echo acym_escapeHtml(acym_translationSprintf('ACYM_CMS_USER', ACYM_CMS_TITLE)); ?>
					</div>
                <?php } ?>
			...
			<div class="grid-x cell align-middle acym__listing__row">
				...
				<div class="grid-x medium-auto small-11 cell acym__listing__title__container">
					<div class="grid-x cell small-9 medium-4 xlarge-3 acym__listing__title">
						<a class="cell auto" href="<?php echo acym_escapeUrl($linkUser); ?>">
							<div><?php echo acym_escapeHtml($user->email); ?></div>
						</a>
					</div>
                    ...
                    if (!empty($user->fields)) {
                        foreach ($user->fields as $field) {
							...
							<div class="medium-auto hide-for-small-only cell">
                                <?php echo acym_escapeHtml($field); ?>
							</div>
                            <?php
                        }
                    }
					...
					<div class="acym__users__subscription medium-auto small-11 cell">
                        ...
                        if (!empty($data['usersSubscriptions'][$user->id])) {
                            ...
                            foreach ($data['usersSubscriptions'][$user->id] as $oneSub) {
                                ...
                                $classes .= $newvalue === 0 ? 'acymicon-circle' : 'acymicon-radio-button-unchecked';
                            ...
                            if ($counter > 5 && $subscriptionsCount !== 6) {
                                $counter = $counter - 5;
                                echo '<span class="acym__user__show-subscription acymicon-stack" data-iscollapsed="0" acym-data-value="'.acym_escape($counter).'">
										<i class="acym__user__button__showsubscription acymicon-circle acymicon-stack-2x"></i>
										<span class="acym__listing__text acym__user__show-subscription-bt acymicon-stack-1x">+'.acym_escapeHtml($counter).'</span>
									</span>';
                            }
                        }
                        ?>
                    ...
                    <?php if (acym_isAdmin()) { ?>
						<div class="cell hide-for-small-only medium-1">
                            ...
                            if (empty($user->cms_id)) {
                                ...
                                echo '<a href="'.acym_escapeUrl(acym_getCmsUserEdit($user->cms_id)).'" target="_blank">';
                                echo acym_escapeHtml($user->cms_username).'<br />';
                                echo acym_escapeHtml(acym_translation('ACYM_ID').': '.$user->cms_id);
                                echo '</a>';
                            }
                            ?>
						</div>
					...
					<div class="large-1 hide-for-small-only hide-for-medium-only cell text-center acym__icon__table">
						<a href="<?php echo acym_escapeUrl($linkUser); ?>"><i class="acymicon-pencil" title="<?php echo acym_escape(acym_translation('ACYM_EDIT')); ?>"></i></a>
                        <?php if (acym_isAdmin()) { ?>
							<a><i class="acymicon-download fastActions"
							      data-action="export"
							      data-acy-elementid="<?php echo acym_escape($user->id); ?>"
							      title="<?php echo acym_escape(acym_translation('ACYM_EXPORT')); ?>"></i></a>
                        <?php } ?>
					...
					<div class="text-center medium-shrink hide-for-small-only acym__listing__text acym__listing__id"><?php echo acym_escapeHtml($user->id); ?></div>
				</div>
			</div>
            <?php
        }
