        ...
        <?php foreach ($data['subscriptions'] as $oneSubscription) { ?>
			<div class="grid-x cell align-middle acym__listing__row">
				<div class="grid-x small-6 large-8 cell acym__users__display__list__name">
					...
					<h6 class="cell auto"><?php echo acym_escapeHtml($oneSubscription->name); ?></h6>
					<span class="cell medium-auto">
					<?php
                    if (acym_isDateValid($oneSubscription->subscription_date)) {
                        echo acym_escapeHtml(acym_date($oneSubscription->subscription_date, acym_translation('ACYM_DATE_FORMAT_LC2')));
                    } else {
                        echo acym_escapeHtml(acym_translation('ACYM_INVALID_DATE'));
                    }
                    ?>
				</div>
				...
				<div acym-data-id="<?php echo intval($oneSubscription->id); ?>"
					...
					<span><?php echo acym_escapeHtml(acym_strtolower(acym_translation('ACYM_UNSUBSCRIBE'))); ?></span>
				</div>
			</div>
        <?php } ?>
    <?php } ?>
