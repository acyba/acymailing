...
<form id="acym_form" action="<?php echo acym_escapeUrl(acym_completeLink(acym_getVar('cmd', 'ctrl'))); ?>" method="post" name="acyForm" data-abide novalidate>
	...
	<div class="cell grid-x">
		...
		<div class="acym__content grid-x cell xxlarge-7 xlarge-9" id="acym__automation__summary">
            <?php
            $data['workflowHelper']->display($this->steps, 'summary');
            if (!empty($data['id'])) {
                ?>
				<div class="acym__automation__summary__info cell grid-x acym__content margin-top-2">
					<h6 class="cell acym__title acym__title__secondary"><?php echo acym_escapeHtml(acym_translation('ACYM_INFORMATION')); ?></h6>
					...
					<div class="cell acym__automation__summary__information__one"><span class="acym__automation__summary__information__one__title"><?php echo acym_escapeHtml(
                                acym_translation(
                                    'ACYM_NAME_SUMMARY'
                                )
                            ); ?></span> : <?php echo acym_escapeHtml($data['automation']->name); ?></div>
					<div class="cell acym__automation__summary__information__one"><span class="acym__automation__summary__information__one__title"><?php echo acym_escapeHtml(
                                acym_translation(
                                    'ACYM_DESCRIPTION'
                                )
                            ); ?></span> : <?php echo acym_escapeHtml($data['automation']->description); ?></div>
				</div>
				<div class="acym__automation__summary__filters cell grid-x margin-top-2 acym__content">
					<h6 class="cell acym__title acym__title__secondary"><?php echo acym_escapeHtml(acym_translation('ACYM_TRIGGERS')); ?></h6>
					<div class="cell acym__automation__summary__information__one"><span class="acym__automation__summary__information__one__title"><?php echo acym_escapeHtml(
                                acym_translation(
					...
					<div class="cell acym__automation__summary__information__one">
                        ...
                        echo implode(
                            '<br /><span class="acym__automation__summary__information__one__title">'.acym_escapeHtml(acym_translation('ACYM_OR')).'</span><br />',
                            $data['step']->triggers
                        );
                        ?>
					</div>
				...
				<div class="acym__automation__summary__actions cell grid-x margin-top-2 acym__content">
					<h6 class="cell acym__title acym__title__secondary"><?php echo acym_escapeHtml(acym_translation('ACYM_CONDITIONS')); ?></h6>
					<div class="cell acym__automation__summary__information__one grid-x">
                        ...
                        if (!empty($data['condition']->conditions)) {
                            ...
                            echo '<strong class="acym__color__red cell text-center">'.acym_escapeHtml(acym_translation('ACYM_SELECT_CONDITIONS')).'</strong>';
                        }
                        ?>
					</div>
				</div>
			...
			<div class="acym__automation__summary__actions cell grid-x margin-top-2 margin-bottom-2 acym__content">
				<h6 class="cell acym__title acym__title__secondary"><?php echo acym_escapeHtml(acym_translation('ACYM_ACTIONS')); ?></h6>
				<div class="cell acym__automation__summary__information__one grid-x">
                    <?php
                    if (!empty($data['action']->actions)) {
                        ...
                        echo '<strong class="acym__color__red cell text-center">'.acym_escapeHtml(acym_translation('ACYM_SELECT_ACTIONS')).'</strong>';
                    }
                    ?>
				</div>
			</div>
			<div class="acym__automation__summary__actions cell grid-x margin-top-1  margin-bottom-2 acym__content">
				<h6 class="cell acym__title acym__title__secondary"><?php echo acym_escapeHtml(acym_translation('ACYM_ACTIONS_TARGETS')); ?></h6>
				<div class="cell acym__automation__summary__information__one grid-x">
                    <?php
                    if (!empty($data['action']->filters)) {
                        ...
                        echo '<strong class="acym__color__red cell text-center">'.acym_escapeHtml(acym_translation('ACYM_SELECT_FILTERS')).'</strong>';
                    }
                    ?>
				</div>
			</div>
			<div class="cell grid-x grid-margin-x">
                ...
                <?php if (empty($data['id'])) { ?>
					<button type="button" data-task="listing" class="cell shrink button-secondary button acy_button_submit"><?php echo acym_escapeHtml(
                            acym_translation('ACYM_CANCEL')
                        ); ?></button>
					<button type="button" data-task="processMassAction" class="cell shrink button acy_button_submit"><?php echo acym_escapeHtml(
                            acym_translation(
                                'ACYM_PROCESS_MASS_ACTION'
                            )
                        ); ?></button>
                <?php } else { ?>
					<button type="button" data-task="listing" class="cell shrink button button-secondary acy_button_submit"><?php echo acym_escapeHtml(
                            acym_translation(
                                'ACYM_SAVE_EXIT'
                            )
                        ); ?></button>
					<button type="button" data-task="activeAutomation" class="cell shrink button acy_button_submit"><?php echo acym_escapeHtml(
                            acym_translation(
