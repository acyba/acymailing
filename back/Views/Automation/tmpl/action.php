...
<form id="acym_form" action="<?php echo acym_escapeUrl(acym_completeLink(acym_getVar('cmd', 'ctrl'))); ?>" method="post" name="acyForm" data-abide novalidate>
	...
	<div class="acym__content grid-x cell" id="acym__automation__actions">
        <?php
        $data['workflowHelper']->display($this->steps, 'action');
        ?>
        ...
        foreach ($data['actions'] as $key => $action) {
            ...
            ?>
			<template id="acym__automation__actions__template__<?php echo acym_escape($key); ?>">
                <?php echo $action->option; ?>
			</template>
        <?php } ?>
		<h6 class="acym__title acym__title__secondary cell"><?php echo acym_escapeHtml(acym_translation('ACYM_SELECT_YOUR_ACTIONS')); ?></h6>
		<div class="cell grid-x acym__content">
			...
			<button data-filter-type="" type="button" class="button-secondary button medium-shrink acym__automation__add-action margin-top-2">
                <?php echo acym_escapeHtml(acym_translation('ACYM_ADD_ACTION')); ?>
			</button>
		</div>
            ...
            <?php if (empty($data['id'])) { ?>
				<div class="cell medium-auto grid-x grid-margin-x text-right">
					...
					<button type="button" class="button acy_button_submit medium-shrink cell" data-task="edit" data-step="setActionMassAction">
                        <?php echo acym_escapeHtml(acym_translation('ACYM_SET_ACTIONS_TARGETS')); ?>
					</button>
				</div>
            <?php } else { ?>
