...
<form id="acym_form" action="<?php echo acym_escapeUrl(acym_completeLink(acym_getVar('cmd', 'ctrl'))); ?>" method="post" name="acyForm" data-abide novalidate>
	...
	<div class="acym__content grid-x cell" id="acym__automation__conditions">
        ...
        $data['workflowHelper']->display($this->steps, 'condition');
        ?>
			...
			<div class="cell grid-x acym__content acym__automation__group__condition" data-condition-number="0">
				...
				<div class="cell grid-x margin-top-2">
					<button data-condition-type="" type="button" class="button-secondary button medium-shrink acym__automation__add-condition">
                        <?php echo acym_escapeHtml(acym_translation('ACYM_ADD_CONDITION')); ?>
					</button>
				</div>
			</div>
		</div>
		...
		<h6 class="acym__title acym__title__secondary cell">
            <?php echo acym_escapeHtml(acym_translation('ACYM_SELECT_YOUR_CONDITIONS'));
            acym_info(['textShownInTooltip' => 'ACYM_CONDITIONS_DESC']); ?>
		</h6>
		<div class="cell grid-x grid-margin-x margin-bottom-2" <?php echo $data['type_trigger'] == 'classic' ? 'style="display: none;"' : ''; ?>>
			...
			<p data-condition="user" class="acym__automation__choose__condition medium-shrink cell <?php echo $data['type_condition'] == 'classic' ? '' : 'selected-condition'; ?>">
                <?php echo acym_escapeHtml(acym_translation('ACYM_EXECUTE_CONDITIONS_ON_ONE_SUBSCRIBER')); ?>
			</p>
			<p data-condition="classic"
			   class="acym__automation__choose__condition medium-shrink cell <?php echo $data['type_condition'] == 'classic' ? 'selected-condition' : ''; ?>">
                <?php echo acym_escapeHtml(acym_translation('ACYM_EXECUTE_CONDITIONS_ON_ALL_SUBSCRIBERS')); ?>
			</p>
			<div class="cell auto"></div>
		</div>
			...
			<div class="cell grid-x acym__content acym__automation__group__condition" data-condition-number="0">
				...
				<div class="cell grid-x margin-top-2">
					<button data-condition-type="classic"
					        data-block="0"
					        type="button"
					        class="button-secondary button medium-shrink acym__automation__add-condition"><?php echo acym_escapeHtml(
                            acym_translation('ACYM_ADD_CONDITION')
                        ); ?></button>
				</div>
			</div>
			<button data-condition-type="classic" type="button" class="acym__automation__conditions__or margin-top-1 button button-secondary"><?php echo acym_escapeHtml(
                    acym_translation(
                        'ACYM_OR'
                    )
                ); ?></button>
		...
		<div class="cell grid-x acym__automation__condition__container"
			...
			<button data-condition-type="user" type="button" class="acym__automation__conditions__or margin-top-1 button button-secondary"><?php echo acym_escapeHtml(
                    acym_translation(
                        'ACYM_OR'
                    )
                ); ?></button>
            ...
            <?php if (empty($data['id'])) { ?>
				<div class="cell medium-auto grid-x grid-margin-x text-right">
					...
					<button type="button"
					        ...
					        data-condition="cleanCondition">
                        <?php echo acym_escapeHtml(acym_translation('ACYM_SET_FILTERS')); ?>
					</button>
				</div>
            <?php } else { ?>
