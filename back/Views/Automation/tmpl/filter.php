...
<form id="acym_form" action="<?php echo acym_escapeUrl(acym_completeLink(acym_getVar('cmd', 'ctrl'))); ?>" method="post" name="acyForm" data-abide novalidate>
	...
	<div class="acym__content grid-x cell" id="acym__automation__filters">
        <?php
        $data['workflowHelper']->display($this->steps, 'filter');
        ?>
		<div id="acym__automation__or__example" style="display: none;">
			<h6 class="cell acym__title acym__title__secondary margin-top-1"><?php echo acym_escapeHtml(acym_translation('ACYM_OR')); ?></h6>
			<div class="cell grid-x acym__content acym__automation__group__filter" data-filter-number="0">
				...
				<div class="cell grid-x margin-top-2">
					<button data-filter-type="" type="button" class="button-secondary button medium-shrink acym__automation__add-filter"><?php echo acym_escapeHtml(
                            acym_translation(
                                'ACYM_ADD_FILTER'
                            )
                        ); ?></button>
		...
		<div style="display: none" id="acym__segment__see-users__example">
            <?php acym_modalInclude(
                '<span class="acym__segment__see-users__button cursor-pointer text-underline">'.acym_escapeHtml(acym_translation('ACYM_SEE_USERS')).'</span>',
		...
		<div class="cell grid-x acym__automation__one__filter" id="acym__automation__and__example" style="display: none;">
			<div class="acym__automation__and cell grid-x margin-top-2">
				<h6 class="cell medium-shrink small-11 acym__title acym__title__secondary"><?php echo acym_escapeHtml(acym_translation('ACYM_AND')); ?></h6>
		...
		<h6 class="acym__title acym__title__secondary cell"><?php echo acym_escapeHtml(acym_translation('ACYM_SELECT_TARGETS_ACTIONS')); ?></h6>
		<div class="cell grid-x grid-margin-x margin-bottom-2" <?php echo $data['type_trigger'] == 'classic' ? 'style="display: none;"' : ''; ?>>
			...
			<p data-filter="user" class="acym__automation__choose__filter medium-shrink cell <?php echo $data['type_filter'] == 'classic' ? '' : 'selected-filter'; ?>">
                <?php echo acym_escapeHtml(acym_translation('ACYM_EXECUTE_ACTIONS_ON_ONE_SUBSCRIBER')); ?>
			</p>
			<p data-filter="classic" class="acym__automation__choose__filter medium-shrink cell <?php echo $data['type_filter'] == 'classic' ? 'selected-filter' : ''; ?>">
                <?php echo acym_escapeHtml(acym_translation('ACYM_EXECUTE_ACTIONS_ON_ALL_SUBSCRIBERS')); ?>
			</p>
			<div class="cell auto"></div>
		</div>
		...
		<div class="cell grid-x acym__automation__filter__container"
            ...
            <?php
            foreach ($data['classic_option'] as $key => $filterHtml) {
                if (empty($key)) {
                    continue;
                }
                ?>
				<template id="acym__automation__filter__classic__template__<?php echo acym_escape($key); ?>">
                    <?php echo $filterHtml; ?>
				</template>
            <?php } ?>
			<div class="cell grid-x acym__content acym__automation__group__filter" data-filter-number="0">
				...
				<div class="cell grid-x margin-top-2">
					<button data-filter-type="classic"
					        data-block="0"
					        type="button"
					        class="button-secondary button medium-shrink acym__automation__add-filter"><?php echo acym_escapeHtml(acym_translation('ACYM_ADD_FILTER')); ?></button>
				</div>
			</div>
			<button data-filter-type="classic" type="button" class="acym__automation__filters__or margin-top-1 button button-secondary"><?php echo acym_escapeHtml(
                    acym_translation(
                        'ACYM_OR'
                    )
                ); ?></button>
		...
		<div class="cell grid-x grid-margin-x margin-top-2">
            ...
            <?php } else { ?>
				<div class="cell medium-shrink medium-margin-bottom-0 margin-bottom-1 text-left">
                    <?php acym_backToListing('automation'); ?>
				</div>
				<div class="auto cell"></div>
				<button type="button"
				        class="button button-secondary acy_button_submit medium-margin-bottom-0 margin-bottom-1 medium-shrink cell"
				        data-task="edit"
				        data-step="saveExitFilters">
                    <?php echo acym_escapeHtml(acym_translation('ACYM_SAVE_EXIT')); ?>
				</button>
				<button type="button" class="button acy_button_submit medium-margin-bottom-0 medium-shrink cell" data-task="edit" data-step="saveFilters">
                    <?php echo acym_escapeHtml(acym_translation('ACYM_SAVE_CONTINUE')); ?>
				</button>
            <?php } ?>
		</div>
