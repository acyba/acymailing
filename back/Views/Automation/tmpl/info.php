		...
		<div class="cell grid-x grid-margin-x">
			<div class="medium-12 small-12 cell grid-x acym__content acym__automation__info__first margin-top-2 margin-y">
				<div class="cell medium-6">
					<label for="automation_name"><?php echo acym_escapeHtml(acym_translation('ACYM_NAME')); ?></label>
				...
				<label class="cell">
					<h6 id="acym__automation__info__desc__button" class="cursor-pointer">
						...
						<i class="acymicon-keyboard-arrow-down"></i>
					</h6>
                    <?php if (!empty($data['automation']->admin)) $data['automation']->description = acym_escapeHtml(acym_translation($data['automation']->description)); ?>
					<textarea style="display: none"
					          name="automation[description]" <?php echo empty($data['automation']->admin) ? '' : 'disabled'; ?>  rows="6"
					          class="margin-top-1"><?php echo !empty($data['automation']->description) ? acym_escapeHtml(
			...
			<div class="medium-12 cell grid-x acym__content acym__automation__info__trigger margin-top-2">
				<div class="cell grid-x margin-bottom-2" id="acym__automation__info__choose__trigger__type">
					<input type="hidden"
					       name="type_trigger"
					       value="<?php echo !empty($data['type_trigger']) ? acym_escape($data['type_trigger']) : 'classic'; ?>"
					       id="acym__automation__trigger__type__input">
					<div class="cell auto"></div>
                    <?php
                    $class = '';
                    if (!empty($data['type_trigger']) && $data['type_trigger'] == 'classic') {
                        $class = 'selected-trigger';
                    } elseif (empty($data['type_trigger'])) {
                        $class = 'selected-trigger';
                    }
                    ?>
					<p id="acym__automation__info__group"
					   data-trigger-type="classic"
					   class="shrink cell <?php echo acym_escape($class); ?> margin-right-2">
                        <?php echo acym_escapeHtml(acym_translation('ACYM_CLASSIC_TRIGGER')); ?>
					</p>
					<p id="acym__automation__info__one-user"
					   data-trigger-type="user"
					   class="shrink cell <?php echo (!empty($data['type_trigger']) && $data['type_trigger'] == 'user') ? 'selected-trigger' : ''; ?>"><?php echo acym_escapeHtml(
                            acym_translation(
				...
				<div class="acym__automation__info__choose__trigger cell grid-x grid-margin-x grid-margin-y"
					...
					<div class="cell large-6 acym__content grid-x acym__automation__droppable__classic">
						<h6 class="acym__title acym__title__secondary cell"><?php echo acym_escapeHtml(acym_translation('ACYM_EXECUTE_AUTOMATION')); ?></h6>
						<div class="cell acym__automation__user-trigger__classic acym__automation__trigger__sortable">
                            <?php
                            if (count($data['defaultValues']) < 2) {
                                echo '<div class="cell margin-top-1 acym__automation__drag-here-text">'.acym_escapeHtml(acym_translation('ACYM_DRAG_YOUR_TRIGGERS')).'</div>';
                            }
                            foreach ($data['classic'] as $key => $classic) {
			...
			<div class="cell grid-x margin-top-2">
				...
				<div class="cell medium-auto grid-x grid-margin-x text-right">
					...
					<button class="cell medium-shrink button medium-margin-bottom-0 margin-bottom-1 acy_button_submit button-secondary"
					        type="button"
					        data-task="saveExitInfo"><?php echo acym_escapeHtml(acym_translation('ACYM_SAVE_EXIT')); ?></button>
