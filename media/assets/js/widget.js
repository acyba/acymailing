if (typeof acym_widget === 'undefined') {
    const acym_widget = {
        init: function () {
            this.initSelect();
        ...
        beaverBuilder: function () {
            let self = this;
            jQuery('.widget_acym_subscriptionform_widget').closest('.fl-module-widget').on('click', function () {
                let settingsInitInterval = setInterval(function () {
                    if (jQuery('.acym_toggle_div_title').length === 0) return;

                    clearInterval(settingsInitInterval);
                    self.initSelect();
                }, 200);
            });
        },
