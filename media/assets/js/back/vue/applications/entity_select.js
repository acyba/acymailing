    ...
    function readyFunction($) {
        ...
        function addTotalRecipients(selectedLists) {
            ...
            $.ajax({
                ...
                data: data,
                beforeSend: function () {
                    $('.entity_select__recipients__number-recipients, #entity_select__recipients__span').hide();
                    $('.acym_loader_logo').show();
                }
            }).done(function (result) {
                const parsedResults = acym_helper.parseJson(result);
                $('.entity_select__recipients__number-recipients').html(parsedResults.data.recipients).show();
                $('#entity_select__recipients__span').show();
                $('.acym_loader_logo').hide();
            }).fail(function () {
                $('.entity_select__recipients__number-recipients').html(0);
            });
        }
            ...
            const appVue = new Vue({
                ...
                methods: {
                    ...
                    handleEntities(data) {
                        ...
                        if (this.entitiesToDisplay_available.length < 1) {
                            this.entitiesToDisplay_available = this.entitiesAvailable.slice(0, start.available);
                        }
                        ...
                        if (this.displaySelected && this.columnJoin !== null) {
                            ...
                            if (this.entitiesToDisplay_selected.length < 1) {
                                this.entitiesToDisplay_selected = this.entitiesSelected.slice(0, start.selected);
                            }
                        }
