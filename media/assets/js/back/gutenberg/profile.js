const acym_profileBlock = {
    ...
    initLists: function () {
        for (let [value, label] of Object.entries(acym_lists_profile)) {
            this.lists.push({
                value: value,
                label: label
            });
    ...
    initFields: function () {
        for (let [value, field] of Object.entries(acym_fields_profile)) {
            this.fields.push({
                value: value,
                label: field.name
            });
    ...
    registerBlock: function (blocks) {
        const icon = this.el('span', {
            class: 'dashicon dashicons dashicons-admin-users',
            style: {
                color: '#2199e8'
            }
        });
        blocks.registerBlockType('acymailing/profile', {
            apiVersion: 2,
            title: ACYM_JS_TXT.ACYM_ACYMAILING_PROFILE_FORM,
            icon: icon,
            category: 'acymailing',
            edit: this.edit
        });
    ...
    edit: function (props) {
        let blockProps = acym_profileBlock.useBlockProps();
        let self = acym_profileBlock;
        ...
        return self.el('div', blockProps, self.el(Fragment, {}, self.el(InspectorControls, {}, self.el(
            ...
            self.selectEdit(ACYM_JS_TXT.ACYM_DROPDOWN_LISTS, props, 'listsdropdown', 'listsdropdown'),
...
window.addEventListener('DOMContentLoaded', () => {
    acym_profileBlock.init(window.wp.blocks, window.wp.element, window.wp.blockEditor, window.wp.components);
});