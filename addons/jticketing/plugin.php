...
class plgAcymJticketing extends AcymPlugin
{
    ...
    public function __construct()
    {
        ...
        $this->addonDefinition = [
            ...
            'documentation' => 'https://docs.acymailing.com/addons/joomla-add-ons/jticketing',
            'category' => 'Event management',
            'level' => 'starter',
        ];
        $this->installed = acym_isExtensionActive('com_jticketing');

        acym_loadLanguageFile('com_jticketing', JPATH_SITE);
        $this->pluginDescription->name = acym_translation('COM_JTICKETING');
        ...
        if ($this->installed) {
            $this->displayOptions = [
                ...
                'startdate' => ['Start DateTime', false],
                'enddate' => ['End DateTime', false],
    ...
    public function insertionOptions(?object $defaultValues = null): void
    {
        ...
        $this->categories = acym_loadObjectList(
            ...
            FROM `#__categories` 
            WHERE extension = "com_jticketing"'
        );
        ...
        if (ACYM_J37) {
            $customFields = acym_loadObjectList(
                'SELECT id, title 
                FROM #__fields 
                WHERE context = "com_jticketing.event" 
                    AND state = 1 
        ...
        foreach ($tags as $oneTag => $parameter) {
            ...
            if (!empty($selectedArea)) {
                if (strpos($oneTag, '{'.$this->name.'_tags') === 0) {
                    $query .= 'JOIN #__contentitem_tag_map AS tags ON element.id = tags.content_item_id AND tags.type_alias = "com_jticketing.event"';
            ...
            if (!empty($parameter->min_publish)) {
                ...
                $where[] = 'element.`created` >= '.acym_escapeDB($parameter->min_publish);
            }
            if (!empty($parameter->min_startdate)) {
                ...
                $where[] = 'element.`startdate` >= '.acym_escapeDB($parameter->min_startdate);
            }
            ...
            if ($parameter->startdate === 'upcoming') {
                $where[] = 'element.startdate >= UTC_TIMESTAMP()';
            }
            if ($parameter->startdate === 'past') {
                $where[] = 'element.startdate < UTC_TIMESTAMP()';
            }
    ...
    protected function groupByCategory(array $elements): array
    {
        ...
        $idsWithCatids = acym_loadObjectList('SELECT `id`, `catid` FROM #__jticketing_events WHERE `id` IN ('.implode(', ', $elements).')');
        usort(
            $idsWithCatids,
            function ($a, $b) {
    ...
    public function replaceIndividualContent(object $tag): string
    {
        $query = 'SELECT element.*, concat(m.path,"/",m.source) as images,`user`.`name` AS authorname, cat.`title` AS category_title, cat.`access` AS category_access
                    ...
                    LEFT JOIN #__tj_media_files_xref AS mxref 
                        ON mxref.client_id = element.id and mxref.client="com_jticketing.event"
                    LEFT JOIN #__tj_media_files AS m 
                        ON m.id = mxref.media_id
                    WHERE element.state = 1
        ...
        $link = 'index.php?option=com_jticketing&view=event&id='.$element->id;

        $menuId = $this->getParam('itemid');
        if (empty($menuId)) {
            $app = Factory::getApplication();
            $menu = $app->getMenu();
            $eventMenu = $menu->getItems('link', 'index.php?option=com_jticketing&view=events&layout=default&catid=0', true);
            if (!empty($eventMenu->id)) {
                $menuId = $eventMenu->id;
            }
        }
        ...
        $siteTimeZone = $config->get('offset');

        $date = new DateTime('now', new DateTimeZone($siteTimeZone));

        $pattern = '/[-+]\d+/';
        $shortTimeZone = $date->format('T');
        if (preg_match($pattern, $shortTimeZone)) {
            $shortTimeZone = 'UTC '.$shortTimeZone;
        }
        $startdate = HTMLHelper::date($element->startdate, $com_params->get('date_format_show'), true).' ('.$shortTimeZone.')';
        $varFields['{startdate}'] = $startdate;

        $enddate = HTMLHelper::date($element->enddate, $com_params->get('date_format_show'), true).' ('.$shortTimeZone.')';
        ...
        $varFields['{intro}'] = $this->fixDivStructure($element->long_description);
        if (in_array('intro', $tag->display)) {
            $contentText .= $varFields['{intro}'];
        }

        if (in_array('startdate', $tag->display)) {
            $customFields[] = [
                $varFields['{startdate}'],
                acym_translation('ACYM_START_DATE'),
            ];
        }
        if (in_array('enddate', $tag->display)) {
            $customFields[] = [
                $varFields['{enddate}'],
        ...
        $varFields['{cat}'] = '<a href="'.$this->finalizeLink('index.php?option=com_jticketing&view=category&id='.$element->catid, $tag).'" target="_blank">'.acym_escapeHtml(
                $category
            ).'</a>';
        ...
        $tags = acym_loadObjectList(
            ...
            WHERE map.type_alias = "com_jticketing.event"
