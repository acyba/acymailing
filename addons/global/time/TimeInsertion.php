...
trait TimeInsertion
{
    ...
    public function textPopup(): void
    {
        ...
        echo '<div class="acym__popup__listing text-center grid-x">
                <h1 class="acym__title acym__title__secondary text-center cell">'.acym_escapeHtml(acym_translation('ACYM_TIME_FORMAT')).'</h1>';

        $k = 0;
        foreach ($others as $tagname => $tag) {
            echo '<div class="grid-x medium-12 cell acym__row__no-listing acym__listing__row__popup text-left" onclick="setTag(\''.acym_escape($tagname).'\', jQuery(this));" >
                    <div class="cell medium-6 small-12 acym__listing__title acym__listing__title__dynamics">'.acym_escapeHtml(acym_translation($tag)).'</div>
                    <div class="cell medium-6 small-12 acym__listing__title acym__listing__title__dynamics">'.acym_escapeHtml(acym_getDate(time(), acym_translation($tag))).'</div>
                 </div>';
            $k = 1 - $k;
        }
